...
Agents that may run shell commands should also carry the `REPO_SAFETY` preamble from `workflows/_template.mjs` — `mktemp -d` rather than a shared path, `cd "$DIR" || exit 1`, and a `git rev-parse --show-toplevel` assertion before anything destructive. Their prompt should also name where they may write: one sentence, and the only control in force *during* a run the guard can only bracket. The prompt prevents the common case (an agent that would have complied and was never told); the preamble is documentation; the guard is the control that still works when instruction fails.
