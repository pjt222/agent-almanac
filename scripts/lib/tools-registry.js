 ...
 * refuses (throws) anything else rather than guessing: a block scalar (`>-`, `|`), a nested
 * key, an unknown field, a missing required field, a duplicate id. The generator
 * (`generate-readmes.js`) reads the same file through this same module, so the two renderings
 ...
 *   superseded_by id of the replacement, or null (required when deprecated)
 *   description   neutral, name-first, one clause — the README table's column
 *   need          one need-first sentence — the ONLY line CLAUDE.md renders
 ...
 *   verify        the exact self-test invocation
...
/** Strip one layer of matching quotes; leave anything else as written. */
...
export function schemaErrors(entries, declaredTotal = undefined) {
  ...
  // superseded_by must name an existing id
  const ids = new Set(entries.map((e) => e.id));
  for (const e of entries) if (e.superseded_by && !ids.has(e.superseded_by)) errors.push(`${e.id}: superseded_by \`${e.superseded_by}\` names no entry`);
...
/**
 * Registry vs disk, both directions, as two named lists plus the schema errors. Never a bare
 * boolean: `fileWithoutRow` and `rowWithoutFile` are different defects with different fixes.
 */
export function checkParity(root, entries) {
  const onDisk = readdirSync(join(root, TOOLS_DIR))
    .filter((name) => !NOT_TOOLS.has(name) && statSync(join(root, TOOLS_DIR, name)).isFile())
    .map((name) => `${TOOLS_DIR}/${name}`)
    .sort();
...
/** The CLAUDE.md index: one need-first line per ACTIVE tool, `not_for` as a suffix, grouped by tag. */
export function renderClaudeBlock(entries) {
  const active = entries.filter((e) => e.status === 'active');
  const order = ['review', 'translation', 'measurement', null];
  const lines = [];
...
export function renderReadmeTable(entries) {
  const rows = entries.map((e) => {
    ...
    return `| \`${e.path.replace(/^tools\//, '')}\` | ${e.language} | ${e.description}${status} | \`${e.verify}\` |`;
  });
