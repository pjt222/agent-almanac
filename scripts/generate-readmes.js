...
function generateSecuritySurface() {
  ...
  // the filenames in the sentence derive from this array, so the word and the list cannot
  // disagree (round-2 S2: a literal "Two" beside derived numbers is the static-prose defect this
  // function polices for scripts/). The membership itself is hand-maintained -- the registry has
  // no field for reach -- and each id is checked against the registry.
  ...
  return [
    ...
    '',
    `- **${treeLabel}**: ${nonDoc.length === 0 ? 'Markdown and YAML only' : `mostly Markdown and YAML, plus **${nonDoc.length} files that are not** (${nonDocExtensions.join(', ')})${executable.length ? ` — ${executable.length === 1 ? 'one of them an executable script, ' : `${executable.length} of them executable scripts: `}${executable.map((f) => `\`${f}\``).join(', ')}` : ''}`}. All of it ships — counted from the commit, which is what a release is packed from; a local \`npm pack\` packs the working tree instead and can include files this count excludes. ${declaring} of ${ids.length} skills (~${share}%) declare \`Bash\` in their \`allowed-tools\`, meaning they instruct AI agents to execute shell commands when followed. Review any skill before letting an agent execute it.`,
    ...
    `- **Tools** (\`tools/\`): ${toolRows.length} operator utilities (${languagePhrase}) catalogued in \`tools/_registry.yml\`, none shipped in the published package and none a merge gate. Each carries a self-test: ${toolsInCi} run in the non-required \`tools-verify\` job, ${toolsSkipped} skipped there with the reason recorded in the registry. ${beyond.length} act beyond this checkout (the tool itself makes a network request or opens a listening socket; a path the caller names is the caller's reach): ${beyondPhrase}.`,
