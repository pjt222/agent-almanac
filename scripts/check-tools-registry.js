 ...
 *                                                   being checked
 *
 * Exit 0 clean; 1 on any schema error, any file without a row, any row without a file, or any
 * self-test that exits non-zero; 2 when the registry cannot be read or parsed (never a pass).
 * Dependency-free: the reader is `scripts/lib/tools-registry.js`, not js-yaml.
 */
...
export function main(argv, io = console, root = ROOT, run = spawnSync) {
  ...
  const summary = `${reg.entries.length} row(s) (${active} active) against ${reg.entries.length - reg.rowWithoutFile.length + reg.fileWithoutRow.length} file(s) under tools/, both directions`;
  // `OK:` only on a clean pass: validate-integrity.sh requires that line as well as exit 0, so a
  // checker that exits 0 having done nothing cannot read as a pass (its round-1 finding 19).
  ...
  if (verify) {
    ...
    for (const e of reg.entries) {
      ...
      ran += 1;
      if (r.status === 0) io.log(`OK: ${e.id}: \`${e.verify}\` exit 0`);
