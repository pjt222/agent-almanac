...
| `watch-checks.sh` | bash | Waits for the checks on a pull request or a commit to settle, printing each context once as it settles, then the full list and a verdict line; exit 0 / 1 / 2 | `bash tools/watch-checks.sh --verify` |
...
| `watch-checks.sh` | Waits for the checks on a PR head or a commit to settle, printing each context once as it settles with the elapsed time, then the full list and a verdict line; exits 0 (every context pass or skipping), 1 (a failure), 2 (no verdict: arguments, three consecutive fetch failures, timeout). `gh pr checks --json` and the check-runs API fold into one vocabulary, fail-closed — a conclusion it has never seen is a failure. `--min-polls` (default 3) closes the trap of a freshly pushed head whose runs are created over the first minute, so "nothing pending" at poll 1 is not "settled". Replaces a loop typed five times in one session; `gh pr checks --watch` prints every result twice and has no commit mode. |
...
bash tools/watch-checks.sh --verify           # canned answers in both API shapes: once-each, the --min-polls guard, every fold, 2 never 0 (19 cases)
