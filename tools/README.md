...
| `merge-pr.sh` | bash | Merges a reviewed, green pull request from a throwaway seat branch (main here is held by a worktree), reads the verdict from the API rather than gh's exit, detaches on the merged base and deletes the branches; exit 0 merged, 1 refused or nothing merged, 2 could not run, 3 merged but cleanup incomplete | `bash tools/merge-pr.sh --verify` |
