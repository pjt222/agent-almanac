...
| `refresh-untranslated-stubs.mjs` | Brings the untranslated stubs of one content id back to a byte copy of English — English's whole frontmatter plus the six translation fields the stub already carries, verbatim, then English's body — after a frozen fence or anything else in the English file changes (#789). Reads `translator:` from each mirror at the moment it decides to write, never from a list, and refuses any value but the scaffolder's literal (read through `translator-stamp.mjs`, so it lives in one place); a refusal names the file and value and is exit 1 only when `--locale` asked for that mirror, since the six compressed locales are never stubs. `--verify` exits 1 naming the first differing line of any stub that is not English-plus-the-six; `--stamp <sha>` is the separate second commit — it records the commit that carries the bytes, quoted so an all-digit sha cannot parse as an integer, and refuses a stub whose bytes no longer match. Zero mirrors is exit 2, not a clean 0. The one tool here with an npm alias (`refresh:stubs`), because the propagation recipe in CLAUDE.md is written as npm commands; it is still not a gate |
