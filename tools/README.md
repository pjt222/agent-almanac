...
| `fact-sheet.sh` | Assembles a fact sheet from a spec of `LABEL :: COMMAND` lines: every command runs as `bash -c` from the git toplevel (or `--root DIR`), stdin from `/dev/null`, stderr merged, and the sheet records the label, the command and the output verbatim — indented, `(no output)` for an empty one, a non-zero exit noted on the label line and never as an indented line, so the block shape the #813 fill scripts parse is unchanged. Two phases: the whole spec is parsed first and every problem (no separator, a second ` :: ` after the split, an empty label or command, a duplicate id, zero facts) is refused at once with exit 2 and OUT untouched; then the facts run into a temporary sibling of OUT that is renamed at the end, so an interrupted run leaves no half sheet. A command's exit never fails the tool: the sheet is the measurement, and a failed measurement is a fact about the run. Exit 0 every fact ran, 2 usage or a refusal. The third typing of one shape in one session (facts-wc.sh, facts-mp.sh, facts.sh), promoted in #816 after the reboot that cleared the scratchpad holding all three. |
