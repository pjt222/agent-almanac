...
| `patch-literal.py` | Applies literal old-to-new edits to files in two phases: every needle's count (default 1, counted sequentially in the running text), the symlink, binary and UTF-8 checks and every `--forbid` residue are decided across ALL files before any byte is written (exit 1, nothing written); then each file goes through a same-directory temporary file and a rename with the mode bits copied, and is read back (exit 3 when a later write fails or a read-back differs, kept apart from 1 so that "not 0" is not read as "nothing written"). Edits arrive as a JSON spec, so nothing is re-escaped between the author and the file (the #807 incident), or as `--replace OLD::NEW` split at the first `::` as `mutation-check.js` does; `--dry-run` prints the unified diff. Promoted from the fill-addendum and finish-body scratchpad scripts after their fifth typing (#812). |
...
python3 tools/patch-literal.py --verify       # 32 fixtures through the real process in a temporary directory: cross-file two-phase, sequential counting both ways, symlink, bytes, mode bits, both exit-3 arms
python3 tools/patch-literal.py FILE --replace 'OLD::NEW' --dry-run   # the counts and the unified diff; nothing written
python3 tools/patch-literal.py --spec edits.json --forbid __          # every file checked, then written and read back; a surviving __PLACEHOLDER__ refuses the lot
