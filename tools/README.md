...
| `append-only.py` | python | Checks that a diff only added text — every original line survives as a prefix of a new line — refusing a commit range that cannot see the working tree and a file the diff never reached | `python3 tools/append-only.py --verify` |
