...
**Expected:** one or more context windows per flag occurrence, each ~500 chars. Multiple occurrences typically share the same reader function but may differ in default or conjunction — inspect each independently.
...
**On failure:** if a context window contains no recognizable reader pattern, the flag may not actually be gate-called — recheck the call-site classification from `sweep-flag-namespace` Step 2. If a window contains a reader name not in this taxonomy, document it as a new variant in your research artifacts and decide whether it warrants a separate handling path.
