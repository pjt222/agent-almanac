...
- You need to report DEFAULT-TRUE flags separately from DEFAULT-FALSE — typically the high-signal subset of any namespace.
...
- **Required**: the binary or bundle file you can read.
- **Required**: a namespace prefix (synthetic example: `acme_*`) that identifies flags belonging to the system under study.
- **Required**: a working documentation set — the running list of flag write-ups your campaign has produced so far.
...
Extract every literal in the binary that matches the namespace prefix, regardless of call-site role. The goal at this step is *coverage*, not classification.
...
**Expected:** a deduplicated candidate list and a frequency-sorted occurrence file. Very high counts (≥10) suggest gate-heavy strings; single-occurrence strings are more likely to be telemetry event names or static labels.

**On failure:** if the unique count is 0, the prefix is wrong (typo, namespace boundary mismatch, the harness uses a different convention than expected). If the count exceeds ~5000, the prefix is too broad — narrow it before continuing or the inventory becomes unmanageable.
...
Consolidate the per-string records into one inventory artifact. CSV or JSONL — pick one and stick to it for diffing across waves.
...
**Expected:** an inventory file with one record per unique gate-bearing flag. The gate count is typically a fraction of `total_unique` (commonly 5–20%), so the two numbers should differ noticeably.

**On failure:** if the inventory is empty or `gate_calls` ≈ `total_unique`, the gate-vs-telemetry disambiguation in Step 2 is producing meaningless splits. Revisit the reader-name regex.

### Step 4: Cross-Reference Against the Documented Set

The completeness metric depends on a documented set — the flags your campaign has already written up in research artifacts. Cross-reference, then report what remains.
...
The completeness metric is `remaining` — when it reaches 0, the documented set covers every gate-bearing flag in the namespace.
...
**Expected:** typical split is 10–20% DEFAULT-TRUE, 80–90% DEFAULT-FALSE. A binary at the extremes (90%+ TRUE or 90%+ FALSE) is unusual and worth investigating — it may indicate a release-stage convention (everything default-on for testing, everything default-off for staged rollout).
...
When `remaining = 0` from Step 4, run a final scan: search for gate-call occurrences of namespace-matching strings that are NOT in the documented set. This catches any flag that was missed by the harvest in Step 1 (e.g., string concatenation that hides the literal from a simple grep).
...
Compare the gate-call hits against `/tmp/sweep-documented.txt`. If any hit references a flag not in the documented set, return to Step 1 with a refined extraction (e.g., handle the dynamic-construction case). If empty: the campaign is complete.

**Expected:** the final scan returns either an empty result (campaign complete) or a small remainder (typically <5 flags, usually surfacing dynamic constructions or alternate readers).
