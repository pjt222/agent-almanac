...
`scripts/sync-discovery-symlinks.sh` reads `skills/_registry.yml`, ensures each registered skill has both its `.claude/skills/<id> -> ../../skills/<id>` (relative) and `~/.claude/skills/<id> -> <almanac>/skills/<id>` (absolute) links, and ensures the `agents` directory symlink at each layer. It **skips teams** (reserved for `TeamCreate` runtime), **never touches non-almanac entries** (a `~/.claude/skills` link pointing outside the almanac — e.g. `peon-ping-*` — is left alone), and skips `_template`. `--report` is read-only and exits non-zero on drift; `--fix` creates missing links, repairs wrong/broken ones, and **removes** almanac-owned stale orphans (a `~/.claude/skills/<id>` link that points into the almanac but is no longer registered) — it `rm`s only such owned links, never external ones. As a safety floor, `--fix` refuses to run (and disables orphan cleanup) if the registry parse looks empty or short versus the on-disk skill dirs, so a malformed registry can never wipe the hub. The global layer is skipped when `~/.claude` is absent (e.g. CI). Projects that chain through `~/.claude/skills` pick up new skills automatically.
