...
name: mindfulness
description: >
  Defensives Situationsbewusstsein, Bedrohungseinschaetzung und mentale Klarheit
  unter Druck kultivieren. Umfasst das Cooper-Farbcode-Bewusstseinssystem,
  Koerpersprache lesen und Absichtserkennung, verbale Deeskalation, bewegte
  Achtsamkeit in oeffentlichen Raeumen, Kampffokus und die OODA-Schleife,
  schnelle Erdungstechniken fuer akuten Stress, kontextspezifische Integration
  sowie fortlaufende Ueberpruefung und Verfeinerung der Bewusstseinsfaehigkeiten.
  Verwenden beim Betreten unbekannter oder potenziell feindlicher Umgebungen,
  bei Bedarf eine Situation auf Sicherheit einzuschaetzen, beim Deeskalieren
  einer verbalen Konfrontation, oder bei der Integration von Bewusstseinspraxis
  in taegliche Bewegung.
license: MIT
allowed-tools: Read
metadata:
...
# Defensive Achtsamkeit kultivieren
...
- Zur Vorbereitung der mentalen Komponente vor Kampfkunsttraining (siehe `tai-chi`, `aikido`)
...
- **Optional**: Fruehere Meditationserfahrung (siehe `meditate`; hilfreich, aber nicht erforderlich)
- **Optional**: Kampfkunst-Trainingshintergrund (siehe `tai-chi`, `aikido`; erweitert physische Reaktionsmoeglichkeiten)
...
**Bei Fehler:** Wenn die Person trotz Deeskalation physisch bedrohlich wird, verschiebt sich die Prioritaet von Deeskalation zu Flucht oder, wenn Flucht unmoeglich ist, zu physischer Verteidigung (siehe `aikido`, `tai-chi`). Nicht jede Situation kann heruntergesprochen werden. Erkennen, wann Deeskalation gescheitert ist, und ohne Zoegern zum Handeln uebergehen.
...
5. Quertraining: Kampfkuenste (siehe `aikido`, `tai-chi`) bauen physische Reaktionsmoeglichkeiten auf; Meditation (siehe `meditate`) baut die ruhige Baseline auf, von der aus Bewusstsein operiert
...
- **Tunnelblick auf die offensichtliche Bedrohung**: Die schreiende Person kann die Ablenkung sein. Sich trainieren, die Peripherie zu scannen, wenn etwas die zentrale Aufmerksamkeit ergreift. Mehrfach-Bedrohungs-Bewusstsein ist der Zweck des Randori-Trainings (siehe `aikido`)
...
- `aikido` — physische Techniken fuer den Fall, dass Deeskalation scheitert; Verschmelzungs- und Umleitungsprinzipien spiegeln verbale Deeskalation
- `tai-chi` — entwickelt verwurzelte Ruhe und Koerperbewusstsein, das sowohl physische Bereitschaft als auch emotionale Regulation unterstuetzt
- `meditate` — baut die Baseline der mentalen Stille auf, von der aus Bewusstsein operiert; sitzende Praxis ergaenzt den aktiven, nach aussen gerichteten Fokus defensiver Achtsamkeit
- `heal` — Erste-Hilfe-Wissen und Stressmanagement sind direkte Anwendungen defensiver Achtsamkeit
- `remote-viewing` — teilt Wahrnehmungsschaerfungstraining; nicht-lokale Bewusstseinsuebungen ergaenzen Umgebungsscannfaehigkeiten
- `awareness` — KI-Selbstanwendungsvariante; bildet Cooper-Farbcodes und OODA-Schleife auf interne Bedrohungserkennung fuer Halluzinationsrisiko und Kontextdegradation ab
