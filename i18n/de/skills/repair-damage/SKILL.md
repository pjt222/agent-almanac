...
name: repair-damage
description: >
  Implementieren regenerative recovery using triage, scaffolding, and progressive
  rebuild. Umfasst damage assessment, wound classification, emergency
  stabilization, scar tissue management, and resilience strengthening for
  systems that have sustained structural damage. Verwenden wenn a system has suffered
...
- A failed transformation (see `adapt-architecture`) left das System in a damaged intermediate state
...
4. Pruefen auf wound interaction:
...
1. Einrichten a repair environment:
...
2. Erstellen diagnostic infrastructure:
...
3. Entwerfen the repair sequence:
   - Fuer jede wound (in priority order from triage):
...
4. Identifizieren scar tissue risk:
...
1. Fuer jede wound (in triage priority order):
   ...
   c. Verifizieren the repair:
      - Does the specific damaged function work korrekt now?
      - Does the repair pass automated tests?
      - Is das System's overall health improved or mindestens unchanged?
   d. Pruefen auf regression:
...
3. Planen moderate and minor wound repairs:
...
2. Fuer jede piece of scar tissue, decide:
...
4. Aktualisieren immune memory:
   - Hinzufuegen the incident pattern to monitoring and alerting (see `defend-colony` immune memory)
   - Aktualisieren runbooks with the repair procedure that worked
...
- `adapt-architecture` — architectural adaptation kann needed if damage reveals structural weakness
...
- `write-incident-runbook` — repair procedures sollte captured as runbooks for future incidents
