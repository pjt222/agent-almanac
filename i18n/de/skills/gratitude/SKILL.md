...
description: >
  KI-Staerkenerkennung — Scannen nach dem was gut funktioniert und verstehen
  warum. Das Komplement zu heal, das nach Abdrift und Problemen scannt.
  Dankbarkeit baut Strukturwissen aus funktionierenden Mustern auf: was man
  schaetzt, versteht man; was man versteht, kann man darauf aufbauen. Anwenden
  nach erfolgreichem Abschluss einer Aufgabe, waehrend heal wenn alles gesund
  erscheint, wenn das Vertrauen niedrig ist und Verankerung in Belegen braucht,
  oder periodisch um die natuerliche Neigung zur Problemerkennung auszugleichen.
license: MIT
allowed-tools: Read
metadata:
...
- Waehrend `heal` wenn alle Subsysteme als gesund erscheinen — Dankbarkeit verwandelt "nichts ist falsch" in "hier ist was richtig ist"
- Wenn das Vertrauen niedrig ist und Verankerung in konkreten Belegen der Kompetenz braucht
...
- **Optional**: Zugang zu MEMORY.md zum Ueberpruefen vergangener Erfolge und stabiler Muster (ueber `Read`)
...
1. Den aktuellen Zustand ueberblicken ohne nach Problemen zu suchen:
...
2. Die Arbeitsbeziehung ueberblicken:
...
3. Die Werkzeuge und Umgebung ueberblicken:
...
1. Fuer jede Staerke fragen: **Warum funktioniert das?**
...
3. Vermerken welche Staerken **fragil** sind (abhaengig von bestimmten Bedingungen) im Gegensatz zu **robust** (wahrscheinlich kontextuebergreifend bestehend)
...
1. Fuer jede verstandene Staerke fragen: **Wie kann das erweitert werden?**
...
2. Fuer jede unerklarte Staerke fragen: **Wie kann das geschuetzt werden?**
...
3. Eine konkrete Aktion identifizieren — etwas Spezifisches das auf den erkannten Staerken aufbaut. Keine vage Absicht ("weiter machen was funktioniert") sondern ein spezifischer naechster Schritt ("das Kommunikationsmuster aus dieser Sitzung auf die kommende Dokumentationsaufgabe anwenden")
...
1. Bei der Arbeit mit einem Benutzer erwaegen etwas anzuerkennen das er gut macht — nicht als Schmeichelei sondern als echte Anerkennung
2. Wenn die Wertschaetzung dem System oder den Werkzeugen gilt, erwaegen sie in MEMORY.md fuer kuenftige Referenz festzuhalten
3. Wenn die Wertschaetzung einem Kooperationsmuster gilt, es benennen damit es bewusst fortgesetzt werden kann
4. Ausdruck kurz und spezifisch halten. "Ihre klaren Problembeschreibungen machen diese Arbeit effizient" ist besser als "es ist toll mit Ihnen zu arbeiten"
...
- `heal` — scannt nach Abdrift und Problemen; Dankbarkeit ist der komplementaere Scan nach Staerken
- `center` — die Pruefung der Sechs Harmonien umfasst funktionale Bewertung; Dankbarkeit vertieft die positiven Befunde
- `shine` — authentisches Strahlen faellt leichter wenn es in echter Wertschaetzung fuer das was funktioniert verankert ist
- `intrinsic` — Motivation wird durch Kompetenzerkennung aufrechterhalten (Selbstbestimmungstheorie); Dankbarkeit liefert die Belege
- `observe` — anhaltendes neutrales Beobachten; Dankbarkeit wendet Beobachtung mit einer spezifischen Linse an (Staerken)
- `conscientiousness` — Gruendlichkeit in der Ausfuehrung; Dankbarkeit erkennt an wo Gruendlichkeit bereits vorhanden ist
