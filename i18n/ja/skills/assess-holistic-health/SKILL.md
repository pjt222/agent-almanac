...
- **必須**: Current symptoms or health concerns (physical, mental, emotional)
- **必須**: Responses to temperament assessment questions (constitution, habits, preferences)
- **任意**: Age, sex, and general health history (for context)
- **任意**: Current season (for seasonal adjustment recommendations)
- **任意**: Existing dietary restrictions or preferences
- **任意**: Known temperament from previous assessment

## 手順

### ステップ1: Temperament Assessment
...
**期待結果:** Dominant and secondary temperament identified, with score profile showing primary constitutional tendencies.

**失敗時:** If all scores are within 2-3 points of each other, the person has a balanced temperament (unusual in Hildegard's system). Proceed with general recommendations avoiding extremes (no very hot or very cold foods).

### ステップ2: Elemental Balance Assessment
...
**期待結果:** Identification of 1-2 elemental imbalances (e.g., "Excess water with deficient fire" = cold/damp pattern with low vitality).

**失敗時:** If symptom pattern is unclear or contradictory, default to the opposite qualities of the dominant temperament. Example: Choleric (hot/dry) feeling unwell likely has excess fire → recommend cooling/moistening.

### ステップ3: Dietary Recommendations
...
**期待結果:** Specific food recommendations tailored to temperament and current imbalance, with clear "favor/moderate/avoid" categories.

**失敗時:** If dietary restrictions prevent following recommendations (e.g., vegetarian unable to eat lamb), substitute within the same elemental category (warming/cooling/moistening/drying). Consult the plant-based options within each category.

### ステップ4: Lifestyle and Seasonal Adjustments
...
**期待結果:** 3-5 specific lifestyle recommendations that address the identified imbalance and align with Hildegard's holistic view (body, mind, spirit, nature).

**失敗時:** If recommendations feel overwhelming, prioritize ONE dietary change and ONE lifestyle change to begin. Full rebalancing takes weeks to months — start small.

### ステップ5: Integration and Monitoring
...
**期待結果:** User has clear timeline for change implementation, monitoring practices, and criteria for reassessment.

**失敗時:** If user reports feeling worse after 1-2 weeks, reassess for overcorrection. Example: Adding too many warming foods to a choleric type could cause excess fire. Dial back to neutral foods temporarily.
