...
### ステップ1: List All Bureaucratic Steps
...
**期待結果:** A numbered list of all bureaucratic steps, categorized and ordered, with notes on shared document requirements.

**失敗時:** If the relocation plan is incomplete or unavailable, build the step list from the destination country's official relocation checklist (e.g., Germany: make-it-in-germany.com, Austria: migration.gv.at, Switzerland: ch.ch/en/moving-switzerland).

### ステップ2: Map Required Documents per Step
...
**期待結果:** A matrix mapping each bureaucratic step to its required documents, with document specifications (original required, copy acceptable, certified translation needed).

**失敗時:** If requirements for a specific step are unclear, check the authority's website directly or call their service line. Requirements can change; do not rely solely on third-party guides older than 12 months.

### ステップ3: Check Current Document Status
...
**期待結果:** A status table for every required document: status (have/copy-only/expired/missing/N-A), validity date, and notes on any issues.

**失敗時:** If document status cannot be confirmed (e.g., documents are in storage or with another party), mark as "unconfirmed" and treat as potentially missing for planning purposes.

### ステップ4: Identify Translation and Apostille Requirements
...
**期待結果:** A translation/legalization matrix showing for each document: translation needed (yes/no), apostille needed (yes/no), estimated cost, and estimated processing time.

**失敗時:** If uncertain whether a specific document needs apostille, contact the destination authority directly. Over-preparing (getting an unnecessary apostille) is better than under-preparing (being turned away at the appointment).

### ステップ5: Generate Action List
...
**期待結果:** A prioritized action list with deadlines, costs, and processing times, plus per-appointment packing lists for documents.

**失敗時:** If processing times are uncertain (common for documents from countries with slower bureaucracies), use worst-case estimates and start the process as early as possible. Flag items where expedited processing is available at additional cost.
