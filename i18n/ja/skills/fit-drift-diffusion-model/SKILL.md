...
- **必須**: Reaction time data with accuracy labels (correct/error) per trial
- **必須**: Subject and condition identifiers for each trial
- **必須**: Choice of DDM variant (basic 3-parameter, full 7-parameter, or hierarchical)
- **任意**: Prior distributions for Bayesian estimation (default: weakly informative)
- **任意**: Number of simulated datasets for parameter recovery (default: 100)
- **任意**: RT filtering bounds in seconds (default: 0.1 to 5.0)

## 手順

### ステップ1: Prepare Reaction Time Data
...
**期待結果:** Cleaned dataframe with no RT outliers, at least 40 trials per subject-condition cell, and accuracy rates between 0.50 and 0.99.

**失敗時:** If trial counts are too low, consider collapsing conditions or removing subjects with excessive missing data. If accuracy is at ceiling (>0.99) or floor (<0.55), the DDM may not be identifiable -- check task difficulty.

### ステップ2: Select DDM Variant
...
**期待結果:** A model variant selected with justification based on trial counts, subject count, and research question.

**失敗時:** If unsure between variants, start with the basic model and add complexity only if residual diagnostics indicate systematic misfit (e.g., error RT distribution mismatch).

### ステップ3: Estimate Parameters
...
**期待結果:** Parameter estimates with standard errors or credible intervals. For Bayesian fits, Gelman-Rubin R-hat < 1.1 for all parameters. Drift rate typically 0.5-4.0, boundary 0.5-2.5, non-decision time 0.15-0.50s.

**失敗時:** If estimation fails to converge, try: (a) tighter parameter bounds, (b) better starting values via grid search, (c) longer chains with more burn-in. If MLE hits boundary values, the model may be misspecified.

### ステップ4: Evaluate Model Fit
...
**期待結果:** QP plot shows predicted quantiles closely tracking observed quantiles for both correct and error RTs. Chi-square test is non-significant (p > 0.05), indicating adequate fit.

**失敗時:** If the model systematically misses fast or slow quantiles, consider adding cross-trial variability parameters (sv, st). If error RT shape is wrong, add starting point variability (sz). Refit with the extended model.

### ステップ5: Compare Models
...
**期待結果:** A clear winner among models with BIC difference > 6, or a justified decision to retain the simpler model when the difference is < 2.

**失敗時:** If models are indistinguishable (BIC difference < 2), prefer the simpler model (parsimony). If the full model wins by a large margin, ensure the basic model was not misspecified due to data issues.

### ステップ6: Validate with Parameter Recovery Simulation
...
**期待結果:** Recovery correlations r > 0.85 for all parameters, bias close to zero (< 5% of parameter range), and RMSE within acceptable bounds for the application.

**失敗時:** Low recovery for a specific parameter usually means: (a) insufficient trials -- increase n_simulated_trials, (b) parameter tradeoffs -- drift rate and boundary can trade off; fix one to test recoverability, (c) flat likelihood surface -- consider reparameterization or Bayesian estimation with informative priors.
