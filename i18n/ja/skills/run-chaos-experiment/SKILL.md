...
- **必須**: Kubernetes cluster (for Litmus or Chaos Mesh)
- **必須**: Steady-state definition (what "normal" looks like)
- **必須**: Hypothesis to test (e.g., "API stays available if one pod crashes")
- **任意**: Observability stack (Prometheus, Grafana) to measure impact
- **任意**: Rollback plan

## 手順

### ステップ1: Define Steady State and Hypothesis
...
**期待結果:** Clear, measurable definition of normal behavior and success criteria.

**失敗時:** If you can't define steady state, observability is insufficient. Add metrics first.

### ステップ2: Set Blast Radius Limits
...
**期待結果:** Experiment has clear boundaries, won't take down entire system.

**失敗時:** If blast radius is too large, narrow scope. Start with one non-critical service.

### ステップ3: Install Chaos Mesh
...
**期待結果:** Chaos Mesh or Litmus running, dashboard accessible.

**失敗時:** Check RBAC permissions. Chaos tools need cluster-wide access.

### ステップ4: Create and Execute Experiment
...
**期待結果:** Pod is killed, Kubernetes restarts it, service continues with minor blip.

**失敗時:** If error rate spikes or service degrades significantly, abort experiment and investigate.

### ステップ5: Analyze Results and Iterate
...
**期待結果:** Learnings captured, fixes implemented, follow-up scheduled.

**失敗時:** If no action is taken post-experiment, chaos engineering becomes theater. Prioritize fixes.

### ステップ6: Graduate to Production (Carefully)
...
**期待結果:** Production experiments run during low-risk windows, with kill switch ready.

**失敗時:** If production experiment causes incident, disable immediately and post-mortem.
