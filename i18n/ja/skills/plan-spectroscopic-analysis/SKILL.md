...
- **必須**: Analytical question (structure identification, quantitation, purity assessment, functional group screening, or reaction monitoring)
- **必須**: Sample description (physical state, approximate quantity, known or suspected compound class)
- **任意**: Available instruments and their capabilities
- **任意**: Budget and time constraints
- **任意**: Safety data (toxicity, reactivity, volatility, light sensitivity)
- **任意**: Prior analytical data (if any results already exist)

## 手順

### ステップ1: Define the Analytical Question
...
**期待結果:** A clearly stated analytical question with defined success criteria and a summary of existing knowledge about the sample.

**失敗時:** If the analytical question is vague ("characterize this sample"), work with the requestor to narrow it down. A vague question leads to unfocused analysis and wasted instrument time.

### ステップ2: Assess Sample Characteristics
...
**期待結果:** A sample characterization summary listing state, quantity, solubility, stability, hazards, and molecular weight range.

**失敗時:** If the sample cannot be characterized adequately (e.g., quantity is too small to test solubility), adopt a conservative approach: start with non-destructive, minimal-sample techniques (Raman, ATR-IR) and assess further after initial results.

### ステップ3: Select Techniques Using the Decision Matrix
...
**期待結果:** A preparation protocol for each selected technique, including solvent choices, quantities needed, and special handling instructions.

**失敗時:** If sample quantity is insufficient for all planned techniques, prioritize based on the information hierarchy from Step 3. If sample is insoluble in all suitable solvents, consider solid-state techniques (ATR-IR, Raman, solid-state NMR, MALDI-MS).

### Step 5: Determine Analysis Sequence and Cross-バリデーション Strategy
...
**期待結果:** A complete, ordered analysis plan with preparation protocols, cross-validation criteria, and contingency provisions documented.

**失敗時:** If the plan cannot be completed due to sample or instrument constraints, document the limitations explicitly and propose the best achievable subset of analyses.
