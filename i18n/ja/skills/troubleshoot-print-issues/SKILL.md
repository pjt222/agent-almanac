...
**期待結果:** Detailed symptom description with photos, parameters, and environmental context.

**失敗時:** If symptoms unclear, print a calibration test (temperature tower, stringing test, or benchy) to reproduce and observe failure systematically.
...
**期待結果:** Failure classified into 1-3 most likely categories.

**失敗時:** If symptoms match multiple categories, prioritize based on failure timing (first layer issues first, then midprint, then top surface).
...
**期待結果:** Root cause identified with supporting evidence (measured temperatures, belt tension, visual inspection).

**失敗時:** If root cause unclear, use elimination method: fix most likely cause, re-test, repeat until resolved.
...
**期待結果:** First layer adheres completely with no lifting.

**失敗時:** Check bed flatness with feeler gauge or mesh leveling; warped bed requires glass/PEI sheet or mesh compensation.
...
**期待結果:** Minimal stringing, thin strings easily removed by hand.

**失敗時:** Check for nozzle partial clog or wet filament (both cause oozing).
...
**期待結果:** No layer shifts in re-print with tightened belts and reduced speeds.

**失敗時:** Check for slicer-generated collisions (part cooling fan hitting model) or electrical issues (stepper driver overheating).
...
**期待結果:** Part stays flat with no corner lifting.

**失敗時:** Material fundamentally unsuitable for printer (ABS on unenclosed printer)—switch to PETG or ASA.
...
**失敗時:** Check for heat creep (cooling fan failure), worn extruder gear, or cracked extruder arm.
...
**失敗時:** Re-calibrate e-steps (may be set too high).
...
**期待結果:** Test print succeeds with issue resolved.

**失敗時:** If test fails, issue not fully resolved or multiple issues present—repeat diagnosis focusing on remaining symptoms.
...
**期待結果:** Issue documented with root cause and solution for knowledge base.

**失敗時:** Even unsuccessful troubleshooting attempts should be logged to avoid repeating failed solutions.
