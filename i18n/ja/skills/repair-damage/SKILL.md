...
- **必須**: Description of the damage (what broke, when, how severely)
- **必須**: Current system state (what's still working, what's not)
- **任意**: Root cause (if known — may not be clear yet)
- **任意**: Pre-damage system state (for comparison)
- **任意**: Available repair resources (time, people, budget)
- **任意**: Urgency (is the system actively degrading or stable-but-damaged?)

## 手順

### ステップ1: Triage — Assess and Classify Wounds
...
**期待結果:** A complete wound inventory classified by severity, with a prioritized repair order that accounts for wound interactions.

**失敗時:** If triage takes too long (the system is actively degrading), skip detailed classification and focus on: "What is the single most critical thing to stabilize?" Fix that first, then return to full triage.

### ステップ2: Emergency Stabilization
...
**期待結果:** The system is stable (not actively degrading) even if degraded. Damage is contained and not spreading. Evidence is preserved for root cause analysis.

**失敗時:** If stabilization fails (damage continues spreading despite containment), escalate to full system fallback: activate disaster recovery, switch to backup system, or gracefully degrade to minimal viable operation. Stabilization that takes too long becomes the disaster.

### ステップ3: Build Repair Scaffolding
...
**期待結果:** A repair environment with diagnostic capability, a sequenced repair plan, and awareness of scar tissue risk.

**失敗時:** If setting up a proper repair environment is too slow (system urgency demands immediate production changes), apply changes directly but with extreme discipline: one change at a time, tested by the available means, rolled back if it doesn't help.

### ステップ4: Execute Progressive Rebuild
...
**期待結果:** Critical and serious wounds are repaired with verified fixes. Emergency patches are removed. The system is restored to functional operation.

**失敗時:** If a repair attempt fails or causes regression, roll back to the previous state and reassess. If multiple repair attempts fail for the same wound, the damage may be too deep for local repair — consider whether the affected component needs full replacement rather than repair (see `dissolve-form`).

### ステップ5: Manage Scar Tissue and Strengthen
...
**期待結果:** Scar tissue is managed (removed, replaced, or accepted with documentation). The system is not only repaired but more resilient than before the damage. Learnings are captured for future incidents.

**失敗時:** If scar tissue management is deprioritized ("it works, don't touch it"), schedule it explicitly. Unmanaged scar tissue accumulates and eventually contributes to the next incident. If the root cause can't be identified, strengthen detection and recovery speed as compensating controls.
