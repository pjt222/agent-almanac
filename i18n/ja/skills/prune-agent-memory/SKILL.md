...
description: >
  保存されたメモリを監査、分類し、選択的に忘却します。タイプ/年齢/アクセス頻度
  によるメモリの列挙と分類、古い参照に対する陳腐化検出、外部アンカーを使用した
  フィデリティチェック、選択的削除のデシジョンツリー、削除しなければ再導出
  されてしまう失敗戦略に対するカウンターメモリ・イノキュレーション、永続メモリ
  にすべきでないものに対する予防フィルタリングルール、および忘却自体がレビュー
  可能な監査証跡をカバーします。メモリが大きくキュレーションされていない場合、
  プロジェクトの状態がメモリの書き込み以来大幅に変化した場合、検索品質が低下
  した場合、または manage-memory と一緒に定期的なメンテナンスとして使用します。
license: MIT
...
- トリガー条件が依然として存在する戦略やパターンを放棄した後 — 削除に頼るのではなく、再導出を防ぐためにイノキュレートする
...
各削除について、エントリ、その分類、および削除の理由を記録します（ステップ7で使用）。

このツリーから DELETE アクションを適用する前に、エントリがイノキュレーション（ステップ5）に値するかどうかを確認します。失敗した戦略、放棄されたアプローチ、危険なパターンは、削除のみではなく削除＋イノキュレートの候補です。
...
### ステップ5: パターンの再導出に対してイノキュレートする

放棄された結論の中には、安全に削除できないものがあります。メモリを生成する条件が依然として存在する場合、削除のみでは失敗します — システムは同じ入力から同じ推論経路を辿って削除されたメモリを再構築してしまいます。これらのケースでは、削除と並行して（または削除の代わりに）再導出を防ぐカウンターメモリを書きます。

**判断ルール — 削除のみ vs 削除＋イノキュレート vs イノキュレートのみ:**

| メモリのカテゴリ | アクション | 理由 |
|---|---|---|
| 陳腐化した事実、古いポインタ、期限切れのコンテキスト | **削除のみ** | 検索のクリーンアップ；再生成されても行動上のリスクはない |
| 失敗した戦略、危険なパターン、トリガーが持続する放棄されたアプローチ | **削除＋イノキュレート** | そうしなければ推論経路が結論を再生成してしまう |
| 後に覆されたが元の根拠が重要な決定 | **イノキュレートのみ** | 元のエントリを保持し、それを指す SUPERSEDED カウンターメモリを追加する |

**SUPERSEDED レコード形式**（auto-memory のフロントマター；他のメモリシステムには構造を適応させる）:
...
SUPERSEDED レコードはメモリディレクトリ内に独自のファイル（例：`superseded_strategy_X.md`）として配置し、アクティブなメモリと並んで検索結果に表示されるようにします。カウンターメモリが実行される変更メカニズムになります：類似のシグナルが到着すると、SUPERSEDED レコードが浮上し、再生成経路をブロックします。

**イノキュレートしない場合：**

- 些細な陳腐化した事実（再生成されても行動上のリスクはない）
- 元のトリガー条件がもはや存在しないメモリ（リネームが完了した、依存関係が削除された、チームが解散した）
- 新しい証拠の下での再導出が積極的に望ましい決定（戦略は将来の状態で機能する可能性があり、再評価されるべき）

**イノキュレーションの衛生：**

- `Pattern` と `Do not re-derive from` を具体的に保ちます。曖昧なカウンターメモリ（「複雑な解決策を試さない」）はノイズです。
- SUPERSEDED エントリに日付を付けます。古いイノキュレーション自体が、根底にある条件が変わると陳腐化する可能性があります — それらは次のプルーニングサイクルでレビュー候補として入ります。
- 放棄されたパターンごとに1つの SUPERSEDED。複数の放棄を1つのカウンターメモリに連鎖させないでください；検索が損なわれます。
- 削除レコードと並んで SUPERSEDED ファイルパスをプルーニングログに追加し、監査証跡が操作の両方の半分を捕捉するようにします。

**期待結果：** ステップ4の削除候補のうち、放棄された戦略や危険なパターンに関わるすべてに対して、対応する SUPERSEDED カウンターメモリファイルが元のエントリの削除前に作成されている。プルーニングログには削除とイノキュレーションの両方が記録されている。アクティブなメモリは無駄なくスリムなまま、再生成経路はブロックされている。

**失敗時：** エントリがイノキュレーションに値するかどうか不明な場合は、デフォルトでイノキュレートします。冗長な SUPERSEDED レコードのコストは小さいですが、再生成された悪いパターンのコストははるかに大きいです。SUPERSEDED リスト自体がノイズになるほど大きくなる場合、それは繰り返される放棄を生み出している上流の条件を調査する合図です — 修正はメモリ層ではなく入力層にあります。
...
- [ ] すべての削除候補に対してイノキュレーション基準が確認された；再導出のリスクが存在する場合は SUPERSEDED カウンターメモリが作成された
- [ ] 予防フィルタールールが文書化または適用された
- [ ] プルーニングログが何を、いつ、なぜ削除したかを記録している — イノキュレートされたエントリの SUPERSEDED ファイルパスとペアになっている
...
- **イノキュレーションなしに失敗戦略を削除する**: それを生み出した条件が依然として存在する場合に、放棄されたアプローチに関するメモリを削除する。システムは同じ入力から同じ推論経路を辿って同じ結論を再生成します。削除はプラセボでした。トリガーが持続する場合は、ステップ5のイノキュレーションを使用します。
