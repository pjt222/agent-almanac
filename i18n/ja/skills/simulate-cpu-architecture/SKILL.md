...
- **必須**: Processor complexity target -- 4-bit, 8-bit, or 16-bit data width; number of general-purpose registers (2-16)
- **必須**: Minimum instruction set -- at least: load, store, add, subtract, bitwise AND/OR, branch, halt
- **任意**: Addressing modes beyond direct (immediate, register-indirect, indexed)
- **任意**: Additional instructions (multiply, shift, compare, jump-and-link for subroutines)
- **任意**: Memory size and word size
- **任意**: Pipeline stages (single-cycle, multi-cycle, or pipelined) -- default is multi-cycle
- **任意**: Implementation medium -- software simulation (Python/JS), HDL (Verilog/VHDL), or paper walkthrough

## 手順

### ステップ1: Define the Instruction Set Architecture
...
**期待結果:** A complete ISA specification where every instruction has a unique opcode, well-defined operand fields, an unambiguous RTL description, and documented flag effects. The instruction encoding must be decodable without ambiguity.

**失敗時:** If the instruction word is too narrow to encode all needed fields, either widen the instruction, reduce the register count, use variable-length instructions (more complex decoding), or split instructions into sub-operations. If opcodes collide, reassign the encoding.

### ステップ2: Design the Datapath
...
**期待結果:** A complete datapath diagram (as a component table and mux table) where every instruction in the ISA has a viable path for its data to flow from source to destination through the ALU, register file, and memory.

**失敗時:** If an instruction cannot be executed with the current datapath (e.g., no path from memory to a register for LOAD), add the missing multiplexer or data path. Walk through each instruction's RTL operation and trace the required signal flow through the datapath.

### ステップ3: Design the Control Unit
...
**期待結果:** A fully connected processor where the control FSM drives the datapath through the correct sequence of phases for each instruction type, and all state transitions occur synchronously on the clock edge.

**失敗時:** If the processor hangs (never reaches HALT) or produces incorrect results, the most likely cause is a control signal error in one specific phase. Use Step 5's cycle-by-cycle trace to isolate the failing cycle. If the PC does not increment correctly, check the FETCH phase wiring. If the wrong register is written, check the register address field extraction from IR.

### ステップ5: Simulate a Small Program and Verify
