...
- **必須**: An insect sighting (live insect in the field or recently encountered specimen)
- **必須**: A camera or smartphone capable of close-up photography
- **任意**: GPS device or smartphone with location services enabled
- **任意**: Notebook or field journal for written observations
- **任意**: Hand lens (10x) for fine detail observation
- **任意**: Ruler or coin for photographic scale reference
- **任意**: iNaturalist account or equivalent citizen science platform account

## 手順

### ステップ1: Record Location, Date, and Weather
...
**期待結果:** A complete context record with date, time, precise location (ideally GPS coordinates), and weather conditions at the time of observation.

**失敗時:** If GPS is unavailable, describe the location relative to landmarks (trail junctions, buildings, water features) with enough detail that the site could be relocated. If weather data is uncertain, estimate temperature range and note "overcast" or "clear" rather than leaving the field blank.

### ステップ2: Document Habitat and Microhabitat
...
**期待結果:** A description of the habitat that places the insect in ecological context, including both the broad landscape and the immediate microhabitat where the insect was found.

**失敗時:** If the microhabitat is difficult to characterize (e.g., insect in flight), note what it was flying near or what it landed on. Record "in flight, 1m above meadow grasses" rather than leaving the field blank.

### ステップ3: Photograph with Diagnostic Quality
...
**期待結果:** At least 3 usable photographs: one dorsal, one lateral, and one with scale reference. Ideally 5 or more images covering multiple angles.

**失敗時:** If the insect moves before multiple angles are captured, prioritize the dorsal view (top-down) as it carries the most diagnostic information for identification. A single sharp dorsal photograph is better than multiple blurry images. If the insect flies away before any photograph, sketch the body shape and note colors from memory immediately.

### ステップ4: Note Behavior and Interactions
...
**期待結果:** At least 3 behavioral observations recorded: activity, movement pattern, and abundance.

**失敗時:** If the insect is encountered briefly (e.g., lands and immediately flies away), record what you did observe and note the observation duration. Even "resting on leaf surface, solitary, flew when approached, observation duration 5 seconds" is useful data.

### ステップ5: Preliminary Identification to Order
...
**期待結果:** A preliminary identification to order (e.g., "Coleoptera — beetle") or an honest "order uncertain" with a physical description.

**失敗時:** If the insect does not clearly match any order in the quick key, record the body shape, wing type, and number of legs. Platforms like iNaturalist will accept "Insecta" as a starting identification, and community identifiers will refine it. An honest "unknown" is always better than a forced guess.

### ステップ6: Submit to Citizen Science Platform
...
**期待結果:** A complete observation submitted to a citizen science platform with photographs, location, date, and preliminary identification, ready for community review.

**失敗時:** If no internet access is available in the field, save all photographs and notes locally with the intention to upload later. Most platforms allow backdated submissions. If you do not have an account, store the record in your personal journal — the data still has value for your own learning and can be uploaded later.
