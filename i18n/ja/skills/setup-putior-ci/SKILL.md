...
- **必須**: GitHub repository with putior annotations in source files
- **必須**: Target file for diagram output (e.g., `README.md`, `docs/workflow.md`)
- **任意**: putior theme (default: `"github"`)
- **任意**: Source directories to scan (default: `"./R/"` or `"./src/"`)
- **任意**: Branch to trigger on (default: `main`)

## 手順

### ステップ1: Create GitHub Actions Workflow
...
**期待結果:** File created at `.github/workflows/update-workflow-diagram.yml`.

**失敗時:** Ensure the `.github/workflows/` directory exists. Adjust the `paths` filter to match where annotated source files live in the repository.

### ステップ2: Write Generation Script
...
**期待結果:** Script at `scripts/generate-workflow-diagram.R` that reads annotations, generates Mermaid code, and replaces content between sentinel markers.

**失敗時:** If `put_merge()` returns empty, check that source paths match the repository layout. Adjust `"./R/"` to the actual source directory.

### ステップ3: Configure Auto-Commit
...
**期待結果:** The workflow only commits when diagrams actually change. No empty commits, no infinite loops.

**失敗時:** If push fails with permission denied, check repository settings: Settings > Actions > General > Workflow permissions must be set to "Read and write permissions".

### ステップ4: Add Sentinel Markers to README
...
**失敗時:** Ensure markers are on their own lines with no leading/trailing whitespace. The script matches exact line content.

### ステップ5: Test the Pipeline
...
**期待結果:** GitHub Actions run completes successfully. The diagram between sentinel markers in README.md is updated with current workflow data.

**失敗時:** Check the Actions log for errors. Common issues:
