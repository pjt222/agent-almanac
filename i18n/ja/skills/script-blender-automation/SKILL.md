...
**期待結果:** Complex geometry generated from mathematical functions
**失敗時:** Check BMesh API calls, verify vertex indexing, ensure faces are manifold
...
**期待結果:** Keyframes inserted, animation plays back correctly
**失敗時:** Check property paths, verify data_path syntax, ensure objects are keyable
...
**期待結果:** Multiple files processed, renders generated for each variant
**失敗時:** Check file paths exist, verify import operators, handle missing materials
...
**期待結果:** Operator appears in search, executes with proper undo support
**失敗時:** Check bl_idname format (lowercase with underscores), verify property types
...
**期待結果:** Interactive operator responds to mouse, left-click confirms, ESC cancels
**失敗時:** Check event types, ensure modal handler is added, handle no active object
...
**期待結果:** Add-on installs via Preferences, operators appear in menus
**失敗時:** Check bl_info format, verify Blender version requirement, ensure all classes listed
...
**期待結果:** Objects created based on external data files
**失敗時:** Validate file format, handle missing fields, provide default values
