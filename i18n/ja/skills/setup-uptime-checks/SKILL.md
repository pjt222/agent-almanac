...
- **必須**: List of HTTP/HTTPS endpoints to monitor
- **必須**: Prometheus instance for metric collection
- **任意**: Multiple geographic probe locations
- **任意**: Status page tool (Statuspage.io, Cachet, custom)
- **任意**: Alert notification channels (PagerDuty, Slack)
...
### ステップ1: Deploy Blackbox Exporter
...
**期待結果:** Blackbox Exporter running and accessible on port 9115.

**失敗時:** Check firewall rules, ensure config volume is mounted correctly.

### ステップ2: Configure Blackbox Modules
...
**期待結果:** Multiple probe modules configured for different check types.

**失敗時:** Validate YAML syntax. Check Blackbox Exporter logs for config errors.

### ステップ3: Configure Prometheus Scrape
...
**期待結果:** Prometheus scraping Blackbox Exporter, metrics visible in Prometheus UI.

**失敗時:** Check Prometheus logs for scrape errors. Verify Blackbox Exporter is reachable.

### ステップ4: Create Uptime Alerts
...
**期待結果:** Alerts fire when endpoints are unreachable or SSL certs expiring.

**失敗時:** Check Prometheus alerts page for rule evaluation errors.

### ステップ5: Build Uptime Dashboard
...
**期待結果:** Dashboard showing uptime %, SSL expiry, response times.

**失敗時:** Check Prometheus data source in Grafana, verify metrics are being scraped.

### ステップ6: Set Up Status Page
...
**期待結果:** Public status page shows current service status and incidents.

**失敗時:** Ensure status page URL is reachable by customers, not behind VPN.
