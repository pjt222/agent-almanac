...
- **必須**: Skill ID (e.g., `create-skill-glyph`) and domain (e.g., `design`)
- **必須**: Visual concept — what the glyph should depict
- **任意**: Reference glyph to study for complexity level
- **任意**: Custom `--glow-sigma` value (default: 4)

## 手順

### ステップ1: Concept — Design the Visual Metaphor
...
**期待結果:** A clear mental sketch of the shape with 2-6 planned layers.

**失敗時:** If the concept is too abstract, fall back to a related concrete object. Review existing glyphs in the same domain for inspiration.

### ステップ2: Compose — Write the Glyph Function
...
**期待結果:** A working R function that returns a list of 2-6 ggplot2 layers.

**失敗時:** If `ggforce::geom_circle` causes errors, ensure ggforce is installed. If coordinates are off, remember the canvas is 100x100 with (0,0) at bottom-left. Test the function interactively:
```r
...
### ステップ3: Register — Map Skill to Glyph
...
**期待結果:** The `SKILL_GLYPHS` list contains the new mapping.

**失敗時:** If the build later reports "No glyph mapped for skill", double-check that the skill ID exactly matches the one in the manifest and registry.

### ステップ4: Manifest — Add Icon Entry
...
**期待結果:** Valid JSON with the new entry placed among its domain siblings.

**失敗時:** Validate JSON syntax. Common mistakes: trailing comma after last array element, missing quotes.

### ステップ5: Render — Generate the Icon
...
**期待結果:** The log shows `OK: <domain>/<skill-id> (seed=XXXXX, XX.XKB)` and the WebP file exists.
...
### ステップ6: Verify — Visual Inspection
...
**期待結果:** A clear, recognizable pictogram with even neon glow on transparent background.
...
### ステップ7: Iterate — Refine if Needed
...
**期待結果:** The final icon passes all verification checks from Step 6.

**失敗時:** If after 3+ iterations the glyph still doesn't read well, consider using a completely different visual metaphor (return to Step 1).
