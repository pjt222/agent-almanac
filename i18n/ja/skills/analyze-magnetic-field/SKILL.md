...
- **必須**: Current distribution specification (geometry, current magnitude and direction)
- **必須**: Region of interest where the field is needed (observation points or volume)
- **任意**: Material properties (relative permeability, B-H curve data, coercivity, remanence)
- **任意**: Desired accuracy level (exact integral, multipole expansion order, numerical resolution)
- **任意**: Visualization requirements (2D cross-section, 3D field lines, magnitude contour map)

## 手順

### ステップ1: Characterize Current Distribution and Geometry
...
**期待結果:** A complete geometric description of the current distribution with coordinate system chosen, symmetries cataloged, and current continuity verified.

**失敗時:** If the geometry is too complex for a closed-form parametric description, discretize into short straight segments (numerical Biot-Savart). If current continuity is violated, add displacement current or return charge accumulation terms before proceeding.

### ステップ2: Select Appropriate Law
...
**期待結果:** A justified choice of method with a clear statement of why the chosen law is appropriate for the problem's symmetry level.

**失敗時:** If Ampere's law is chosen but the symmetry is insufficient (B cannot be extracted from the integral), fall back to Biot-Savart. If the source geometry is too complex for analytic Biot-Savart, discretize numerically.

### ステップ3: Set Up and Evaluate Field Integrals
...
**期待結果:** An explicit expression for B(r) at the observation points, with correct units (Tesla or Gauss) and a convergence check for numerical results.

**失敗時:** If the integral diverges, check for a missing regularization (e.g., the field on the wire itself diverges for an infinitely thin wire -- use finite wire radius). If numerical results oscillate with N, the integrand has a near-singularity that requires adaptive quadrature or analytical subtraction of the singular part.

### ステップ4: Check Limiting Cases
...
**期待結果:** All limiting cases match. The field has the correct units, symmetry, and asymptotic behavior.

**失敗時:** A failed limit indicates an error in the integral setup or evaluation. The most common causes are: wrong sign in the cross product, missing factor of 2 or pi, incorrect limits of integration, or a coordinate system mismatch between source and field point parameterizations.

### ステップ5: Incorporate Magnetic Materials and Visualize
...
**期待結果:** A complete field solution including material effects where relevant, with a visualization that shows closed field lines consistent with div B = 0 and qualitative behavior matching physical intuition.

**失敗時:** If field lines do not close, the calculation has a divergence error -- recheck the integral or numerical method. If the material introduces unexpected field amplification, verify that mu_r is applied only inside the material volume and that boundary conditions are correctly enforced at every interface.
