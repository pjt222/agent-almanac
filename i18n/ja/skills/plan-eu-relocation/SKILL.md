...
**期待結果:** A structured profile document containing nationality status, employment classification, move type, and all fixed dates.

**失敗時:** If nationality or employment status is ambiguous (e.g., dual nationality with one non-EU, or contractor vs. employee distinction unclear), escalate to a legal advisor or the destination country's embassy before proceeding. Do not guess visa requirements.

### ステップ2: Map Dependency Chain
...
**期待結果:** A dependency graph (textual or visual) showing all steps, their prerequisites, and which can run in parallel.

**失敗時:** If dependencies are unclear for a specific country, search for official government sources (e.g., Germany: bmi.bund.de, Austria: oesterreich.gv.at, Switzerland: ch.ch). Do not assume dependencies transfer between countries.

### ステップ3: Create Timeline with Deadlines
...
**期待結果:** A week-by-week timeline spanning from 8-12 weeks before the move to 4-8 weeks after, with each bureaucratic step placed in its execution window.

**失敗時:** If appointment availability is unpredictable (common in large German cities), build in a 2-week buffer and identify alternative offices or early-morning walk-in options.

### Step 4: Identify Country-Specific 手順s
...
**期待結果:** A country-specific procedure list with exact office names, required forms, and typical processing times.

**失敗時:** If destination is a smaller municipality, procedures may differ from the national standard. Check the specific Gemeinde/Kommune website or call their Buergerservice directly.

### ステップ5: Flag High-Risk Items
...
**期待結果:** A risk register with each high-risk item, its deadline, penalty, and mitigation strategy.

**失敗時:** If penalty amounts or deadlines cannot be confirmed through official sources, mark them as "unconfirmed" and recommend direct inquiry with the relevant authority. Do not invent penalty amounts.

### ステップ6: Generate Relocation Plan Document
...
**期待結果:** A complete, structured relocation plan document ready for execution, with all items traceable back to the dependency graph and risk register.

**失敗時:** If the plan is too complex for a single document (e.g., multi-country move with dependents requiring separate visa tracks), split into a master timeline and per-person sub-plans.
