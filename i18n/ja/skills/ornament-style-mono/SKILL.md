...
- **任意**: Specific motif preference (acanthus, palmette, meander, arabesque, etc.)
- **任意**: Rendering style preference (line art, silhouette, woodcut, pen-and-ink, engraving)
- **任意**: Target resolution and aspect ratio
- **任意**: Seed value for reproducible generation

## 手順

### ステップ1: Select Historical Period
...
**期待結果:** A clearly identified period with 2-3 candidate motifs and understanding of why the period's ornament works well (or presents challenges) in monochrome.

**失敗時:** If the user requests a period not in the table (e.g., Celtic, Aztec, Art Deco), research its ornamental vocabulary using WebSearch or WebFetch and construct an equivalent entry with motif list and mono suitability assessment before proceeding.

### ステップ2: Analyze Motif Structure
...
**期待結果:** A structural description like "bilateral symmetry, band-based scaffold, line-filled, interlocking edges" that will inform the prompt.

**失敗時:** If the motif structure is unclear, look up visual references using WebSearch for "[period] [motif] ornament" and analyze the first few results. Speltz's original plates are public domain and widely available online.

### ステップ3: Construct Monochrome Prompt
...
**期待結果:** A prompt of 20-40 words that specifies rendering style, motif, period, composition, and monochrome constraint.

**失敗時:** If the prompt is too vague, add structural specifics from Step 2. If too complex (over 50 words), simplify by removing adjectives and keeping only the structural essentials. Z-Image responds best to clear, specific prompts — avoid abstract or conceptual language.

### ステップ4: Configure Generation Parameters
...
**期待結果:** A complete parameter set ready for generation: resolution, steps, shift, seed strategy.

**失敗時:** If unsure about resolution, default to 1024x1024 (1:1) — it works for most ornamental contexts and is the fastest to generate.

### ステップ5: Generate Image
...
**期待結果:** A generated image and a seed value. The image should show recognizable ornamental forms in monochrome.

**失敗時:** If the MCP tool is unavailable, verify that hf-mcp-server is configured (see `configure-mcp-server` or `troubleshoot-mcp-connection`). If the tool is available but returns an error, simplify the prompt and retry. If the generated image is entirely abstract with no ornamental character, the prompt needs more specific structural language — return to Step 3.

### ステップ6: Evaluate Against Style Criteria
...
**期待結果:** A scored evaluation with specific observations. Most first-generation images will score Adequate on 2-3 criteria.

**失敗時:** If all criteria score Weak, the prompt may be too abstract or too complex. Simplify to the most essential elements: one motif, one rendering style, explicit "monochrome black and white" constraint. Consider switching to a period with higher mono suitability.

### ステップ7: Iterate or Finalize
...
**期待結果:** An improved image after 1-2 iterations, or a decision to accept the current best result.

**失敗時:** If iteration is not improving results, the fundamental prompt concept may not translate well to the model. Try a different motif from the same period, or switch the rendering style entirely (e.g., from line art to silhouette).

### ステップ8: Document the Design
...
**期待結果:** A reproducible record that allows the exact image to be regenerated and its design lineage understood.

**失敗時:** If documentation feels excessive, at minimum record the final prompt and seed — these two values are sufficient to reproduce the image.
