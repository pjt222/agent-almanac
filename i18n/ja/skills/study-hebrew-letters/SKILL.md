...
- **必須**: A specific Hebrew letter to study (e.g., "Aleph," "Shin," "Beth") or a request for the full classification system
- **任意**: Tradition preference (Sefer Yetzirah, Zohar, Hermetic/Golden Dawn)
- **任意**: Focus area (form, sound, number, correspondence, meditation)
- **任意**: Connection to a path on the Tree of Life

## 手順

### ステップ1: Select and Identify the Letter
...
**期待結果:** The letter is identified with its number, category, and primary correspondence. The user understands where it sits within the three-fold classification system.

**失敗時:** If the user names a letter ambiguously (e.g., "Chet" vs. "Cheth" vs. "Het"), confirm by providing the standard value and asking the user to verify.

### ステップ2: Examine the Letter's Form
...
**期待結果:** The user sees the letter not just as an alphabet character but as a visual symbol carrying meaning in its form. The shape itself teaches.

**失敗時:** If form analysis feels subjective, ground it in traditional sources (Sefer ha-Bahir, Otiot de-Rabbi Akiva) where available. Where tradition is silent, present the observation as suggestion rather than doctrine.

### ステップ3: Note Numerical Value and Position
...
**期待結果:** The numerical dimension of the letter is established — both its value and its position on the Tree. The user can connect this letter to gematria analysis and sephirotic study.

**失敗時:** If the Tree of Life path attribution is contested (different systems assign different letters to different paths), present the major systems (GRA, Golden Dawn) side by side rather than choosing one.

### ステップ4: Study Correspondences
...
**期待結果:** A comprehensive correspondence map for the letter. The user sees how the letter connects to cosmology, the body, the calendar, and the symbolic landscape.

**失敗時:** If correspondences conflict between sources, present both and note the recension. Do not silently choose one tradition over another.

### ステップ5: Practice Letter Meditation
...
**期待結果:** The user has engaged the letter through multiple modes — sight (form), sound (chanting), and meaning (contemplation). The letter has become experiential rather than purely intellectual.

**失敗時:** If visualization is difficult, substitute writing: draw the letter slowly and deliberately, multiple times, as a meditative act. Physical engagement with the form can substitute for mental visualization.
