...
- **必須**: Garden hand tools to maintain
- **必須**: Sharpening stone (1000/3000 grit combination stone or diamond plate)
- **必須**: Light machine oil or camellia oil (tsubaki oil)
- **任意**: Wire brush, steel wool (fine grade)
- **任意**: Linseed oil (for wooden handles)
- **任意**: 220-grit sandpaper (for handle refinishing)
- **任意**: Replacement handles or hardware

## 手順

### ステップ1: Know the Eight Essential Tools
...
**期待結果:** Familiarity with the core tool set and each tool's primary function.

**失敗時:** If budget is limited, prioritize in this order: secateurs → trowel → hand fork → hori-hori. These four cover 90% of garden tasks.

### ステップ2: After-Use Quick Clean (2-3 Minutes)
...
**期待結果:** Clean, dry tools returned to designated storage after every use.

**失敗時:** If rust has already started (orange spots on steel), it's not too late. See Step 3 for remediation.

### ステップ3: Monthly Maintenance — Sharpening and Oiling
...
**期待結果:** All edged tools sharp enough to cut cleanly. All metal surfaces lightly oiled.

**失敗時:** If a blade has nicks or chips, more aggressive sharpening is needed — start with a coarser stone (400-600 grit) before finishing with 1000/3000. Deep damage may require professional grinding.

### ステップ4: Rust Remediation
...
**期待結果:** Rust removed, tool functional, protective oil applied.

**失敗時:** If rust has pitted the blade deeply (visible craters), the tool has lost metal and may not hold a sharp edge. Consider replacement — a deeply pitted secateur blade can damage plant tissue, inviting disease.
...
**期待結果:** Smooth, oil-finished handles that shed water and prevent blisters.

**失敗時:** If handles are beyond repair (deep cracks, rot), replacement handles are available for quality tools. Most secateur, fork, and trowel manufacturers sell replacement handles.

### ステップ6: Meditate Checkpoint — Winter Tool Care Ritual
...
**期待結果:** All tools in peak condition, stored properly for winter, gardener settled into the dormant season.

**失敗時:** If winter maintenance is skipped entirely, do a pre-spring emergency cleanup: wire brush all rust, sharpen everything, oil everything. It's never too late to start caring for your tools.
