...
- **必須**: Agent Card (JSON) defining the agent's skills and capabilities
- **必須**: Implementation language (TypeScript/Node.js or Python)
- **必須**: Task execution logic for each skill defined in the Agent Card
- **任意**: Push notification webhook support (`true` or `false`)
- **任意**: Persistent task store (in-memory, Redis, PostgreSQL)
- **任意**: Authentication middleware matching the Agent Card's auth scheme
- **任意**: Maximum concurrent tasks limit

## 手順

### ステップ1: Set Up Project with JSON-RPC 2.0 Handler
...
**期待結果:** An HTTP server that accepts JSON-RPC 2.0 requests and serves the Agent Card.

**失敗時:** If JSON-RPC parsing fails, validate that the request body has `jsonrpc`, `method`, and `id` fields. Return `-32700` (Parse error) for malformed JSON and `-32600` (Invalid Request) for missing required fields.

### ステップ2: Implement Task State Machine
...
**期待結果:** A task store that enforces valid state transitions and maintains history.

**失敗時:** If an invalid state transition is attempted (e.g., `completed` to `working`), return a JSON-RPC error with code `-32002` and a descriptive message. Never silently ignore invalid transitions.

### ステップ3: Add tasks/send and tasks/get Methods
...
**期待結果:** Working `tasks/send`, `tasks/get`, and `tasks/cancel` methods that correctly manage task lifecycle.

**失敗時:** If skill matching fails, return the task in `failed` state with a descriptive message. If the task store is full, return `-32003` (resource exhausted).

### ステップ4: Implement SSE Streaming for tasks/sendSubscribe
...
**期待結果:** SSE streaming that sends real-time status and artifact events as the task progresses.

**失敗時:** If SSE connection drops, the client should be able to reconnect and use `tasks/get` to retrieve the current state. Ensure the task store does not depend on active SSE connections.

### ステップ5: Add Push Notification Webhook Support
...
**期待結果:** Webhook registration and delivery with retry logic.

**失敗時:** Push notification failures must never affect task execution. Log errors and continue. If the webhook URL is persistently unreachable, remove the subscription after max retries.

### ステップ6: Integrate with Agent Card for Discovery
...
**期待結果:** A running A2A server that serves its Agent Card, accepts tasks, and manages their full lifecycle.

**失敗時:** If the Agent Card capabilities do not match the implementation, the startup validation from 6.1 will catch the mismatch. Fix the implementation or update the Agent Card to match.
