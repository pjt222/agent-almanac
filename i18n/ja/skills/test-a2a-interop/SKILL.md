...
- **必須**: Base URL of the A2A agent under test
- **必須**: Authentication credentials (if the agent requires them)
- **任意**: Second agent URL for bidirectional interop testing
- **任意**: Specific skills to test (default: all skills in the Agent Card)
- **任意**: Test timeout per task (default: 60 seconds)
- **任意**: Output format for the conformance report (`json`, `markdown`, `junit`)

## 手順

### ステップ1: Fetch and Validate Agent Cards
...
**期待結果:** Agent Card passes all structural validation checks.

**失敗時:** Record each validation failure with the specific field and reason. Do not abort; continue testing other aspects. An invalid Agent Card is itself a test result.

### ステップ2: Send Test Tasks Covering All Lifecycle States
...
**期待結果:** All lifecycle state transitions work correctly. Tasks complete successfully, cancel cleanly, and multi-turn interaction functions when supported.

**失敗時:** Record the specific state transition that failed, the expected state, and the actual state. Include the full JSON-RPC response in the report for debugging.

### ステップ3: Validate SSE Streaming Responses
...
**期待結果:** SSE stream delivers correctly formatted events in the right sequence, ending with a final terminal event.

**失敗時:** If SSE is advertised but the endpoint returns a non-SSE response, record as a conformance failure. If events arrive out of order, record the sequence. If the stream never terminates, record a timeout.

### ステップ4: Test Error Handling and Edge Cases
...
**期待結果:** All error conditions return appropriate JSON-RPC error codes without crashing the server.

**失敗時:** Record each error handling test that fails. Server crashes during error testing are critical failures that must be fixed before deployment.

### ステップ5: Generate Interoperability Conformance Report
...
**期待結果:** A complete conformance report with pass/fail results, conformance level, and actionable recommendations.

**失敗時:** If the report generation itself fails, output raw test results to stdout as a fallback. The test data should never be lost due to a reporting error.
