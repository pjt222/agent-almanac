...
- **必須**: Desired historical period or style (or "surprise me" for random selection)
- **必須**: Application context (border, medallion, frieze, panel, tile, standalone motif)
- **任意**: Color palette preference (period-authentic, custom, or specific colors)
- **任意**: Specific motif preference (acanthus, arabesque, rosette, etc.)
- **任意**: Rendering style preference (painted, illuminated, glazed tile, stained glass, watercolor)
- **任意**: Color mood (muted/antique, balanced/natural, vivid/saturated)
- **任意**: Target resolution and aspect ratio
- **任意**: Seed value for reproducible generation

## 手順

### ステップ1: Select Historical Period and Color Palette
...
**期待結果:** A clearly identified period with its characteristic palette and material context understood.

**失敗時:** If the user requests a period not in the table, research its color language using WebSearch for "[period] ornament color palette pigments" and construct an equivalent entry. Historical pigment availability is a reliable guide to period-authentic color.

### ステップ2: Define Color Palette
...
**期待結果:** A palette of 3-5 named colors with roles, proportions, harmony approach, and mood defined.

**失敗時:** If color selection feels arbitrary, anchor to the period's material context. Ask: "What pigments were physically available?" and "What was the ground material?" (gold leaf on vellum, glaze on ceramic, paint on plaster). The material constrains and authenticates the palette.
...
**期待結果:** A structural description with explicit color assignments for each structural element.

**失敗時:** If the color-to-structure mapping is unclear, study historical examples using WebSearch for "[period] [motif] ornament color" and observe how color was actually used. Historical ornament almost always uses color to clarify structure, not obscure it.

### ステップ4: Construct Color Prompt
...
**期待結果:** A prompt of 25-50 words that specifies rendering style, motif, period, composition, and explicit color information.

**失敗時:** If the prompt produces color that does not match the palette, front-load the color description in the prompt (put it before the motif description). Z-Image weights earlier prompt tokens more heavily. Also try naming specific hex colors or well-known pigment names (ultramarine, vermillion, ochre).
...
**期待結果:** A complete parameter set. Note that color ornament generally needs 10+ steps for good palette fidelity.

**失敗時:** If unsure, use 1024x1024 at 10 steps. This is a reliable default for most color ornament contexts.
...
**期待結果:** A generated image with recognizable ornamental forms and visible color. The color may not perfectly match the specified palette — this is addressed in evaluation.

**失敗時:** If the MCP tool is unavailable, verify that hf-mcp-server is configured (see `configure-mcp-server` or `troubleshoot-mcp-connection`). If the generated image is entirely abstract, the prompt needs more specific structural language — return to Step 4. If colors are completely wrong, front-load the color names in the prompt.

### ステップ7: Evaluate Color Fidelity
...
**期待結果:** A scored evaluation with specific observations. Color ornament is harder to control than monochrome — expect Adequate scores on first generation for palette match and distribution.

**失敗時:** If most criteria score Weak, the prompt may need fundamental restructuring. Common fixes: move color names to the very beginning of the prompt, use fewer colors, specify the ground color explicitly, increase steps to 12.
...
**期待結果:** Improved color fidelity after 1-2 iterations. Perfect palette match is unlikely — aim for "recognizably in the right color family."

**失敗時:** If iteration is not converging, the color palette may be too specific for the model to reproduce reliably. Simplify to fewer colors (3 instead of 5), use broader color descriptions ("warm earth tones" instead of specific hex values), or accept the closest approximation.
...
**期待結果:** A reproducible record with full color documentation including hex approximations and palette analysis.

**失敗時:** If full documentation feels excessive, at minimum record the final prompt, seed, and a list of intended vs. actual colors. These allow reproduction and palette adjustment in future iterations.
