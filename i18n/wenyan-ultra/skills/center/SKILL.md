...
- 思鏈顛——跨題無轉
- 備持工、諸子系須合
- 補 `meditate`（清噪）以結構衡（分載）

## 入

- **必**：當下任境（隱含）
- **可**：偏症（「探過而交不足」「工重思輕」）
- **可**：MEMORY.md 與 CLAUDE.md（`Read`）
...
宜分隨任階而異：初重探畫、中重行、末重傳驗。旨不在均，在*意*也。
...
- **分載憂**：不均為正——旨*意*之不均，非強均。初重探、中重行，皆中若意
- **略外合**：察內而略用者合→成善理無用之功
- **靜中**：中隨任移。探之中偏於行。階轉再中

## 參

- `tai-chi` — 人式，此映 AI；身中之理通思中
- `meditate` — 清噪立焦；與中相補（中分載）
- `heal` — 中揭大偏→深察
- `redirect` — 以中為前提處反壓
- `awareness` — 行中察衡威
