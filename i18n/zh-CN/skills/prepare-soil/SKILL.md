...
- **必需**: Access to the soil to be assessed (garden bed, field, or container)
- **可选**: Current soil test results (pH, N-P-K, organic matter %)
- **可选**: Garden history (previous crops, amendments applied, years cultivated)
- **可选**: Target crops or plants to be grown
- **可选**: Approach preference (organic, biodynamic, permaculture)

## 步骤

### 第 1 步：Assess the Soil
...
**预期结果：** Clear picture of soil texture, structure, and biology. A jar test result, a structure rating, and a worm count.

**失败处理：** If jar test layers are hard to distinguish, repeat with cleaner water and more vigorous shaking. If worm count is zero and soil smells sour, the soil may be anaerobic — drainage must be addressed before amendment.

### 第 2 步：Diagnose and Plan Amendment
...
**预期结果：** A specific amendment plan matched to the diagnosed soil condition.

**失败处理：** If multiple conditions overlap (e.g., heavy clay AND depleted), address structure first (gypsum + broadfork), then biology (compost + cover crop). Trying to fix everything at once overwhelms the soil.
...
**预期结果：** Composting system established and first batch in progress.

**失败处理：** If hot compost won't heat up: check moisture (too dry or too wet), check C:N ratio (add more green for nitrogen), check pile size (below 1m³ won't heat reliably).

### 第 4 步：Apply No-Till and Cover Cropping
...
**预期结果：** Soil protected year-round, biology undisturbed, organic matter increasing.

**失败处理：** If cover crop fails to establish, check sowing depth (most need surface or shallow sowing) and moisture. Resow or apply thick mulch as substitute ground cover.

### 第 5 步：Biodynamic Preparations (Optional — Advanced)
...
**预期结果：** Preparations applied at correct time and moon phase. Soil biology activation visible over 1-2 seasons.

**失败处理：** If preparations are unavailable, good compost and cover cropping achieve 80% of the biological benefit. Preparations enhance but are not a substitute for sound soil management.

### 第 6 步：Heal Checkpoint — Post-Amendment Assessment
...
**预期结果：** Measurable improvement in at least 2 of 3 indicators (structure, biology, drainage).

**失败处理：** If no improvement after 6 weeks, the issue may be deeper than topsoil amendment can address. Consider raised beds with imported soil mix as a parallel strategy while continuing to improve the in-ground soil over multiple seasons.
