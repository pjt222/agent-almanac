...
- **必需**: putior installed (see `install-putior`)
- **必需**: Target client: Claude Code, Claude Desktop, or both
- **可选**: Whether to also configure ACP server (default: no)
- **可选**: Custom host/port for ACP server (default: localhost:8080)

## 步骤

### 第 1 步：Install MCP Dependencies
...
**预期结果：** Both packages install and load without errors.

**失败处理：** `mcptools` requires `remotes` package. Install it first: `install.packages("remotes")`. If GitHub rate-limits, configure a `GITHUB_PAT` in `~/.Renviron` (add the line `GITHUB_PAT=your_token_here` and restart R). Do **not** paste tokens into shell commands or commit them to version control.

### 第 2 步：Configure Claude Code (WSL/Linux/macOS)
...
**预期结果：** `putior` appears in the MCP server list with status "configured".

**失败处理：** If Claude Code is not in PATH, add it: `export PATH="$HOME/.claude/local/node_modules/.bin:$PATH"`. If the Rscript path is wrong, locate R with `which Rscript` or `ls "/mnt/c/Program Files/R/"`.

### 第 3 步：Configure Claude Desktop (Windows)
...
**预期结果：** Claude Desktop shows putior in its MCP server list. Tools become available in conversation.

**失败处理：** Validate JSON syntax with a JSON linter. Check that the R path exists. Use 8.3 short names (`PROGRA~1`, `R-45~1.0`) if spaces in paths cause issues.

### 第 4 步：Verify All 16 Tools
...
**预期结果：** All 16 tools listed. Core tools return expected results when called with valid inputs.

**失败处理：** If tools are missing, check that putior version is current: `packageVersion("putior")`. Older versions may have fewer tools. Update with `remotes::install_github("pjt222/putior")`.

### 第 5 步：Configure ACP Server (Optional)
...
**预期结果：** ACP server starts on the configured port. `/agents` returns the putior agent manifest. `/runs` accepts natural language requests and returns workflow results.

**失败处理：** If port 8080 is in use, specify a different port. If `plumber2` is not installed, the server function will print a helpful error message suggesting installation.
