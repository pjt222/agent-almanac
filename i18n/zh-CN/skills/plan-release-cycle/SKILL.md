...
- **必需**: Target version number (e.g., v2.0.0)
- **必需**: Desired release date or release window
- **必需**: List of planned features or scope (backlog, roadmap, or description)
- **可选**: Team size and availability
- **可选**: Release strategy preference (calendar-based or feature-based)
- **可选**: Regulatory or compliance requirements affecting release
- **可选**: Previous release velocity or cycle duration data

## 步骤

### 第 1 步：Determine Release Strategy
...
**预期结果：** Release strategy documented with rationale matching project context.

**失败处理：** If the team cannot agree on a strategy, default to calendar-based with a feature-priority list. Time-boxing forces prioritization decisions.

### 第 2 步：Define Milestones
...
**预期结果：** Milestone table with dates, durations, and descriptions for each phase.

**失败处理：** If the timeline is too compressed (stabilization < 1 week), either extend the release date or reduce scope. Never skip stabilization.

### 第 3 步：Set Feature Freeze Criteria
...
**预期结果：** Feature freeze rules documented with exception process and prioritized feature list.

**失败处理：** If P0 features are at risk of missing the freeze date, escalate immediately. Options: extend development phase, split the feature into a smaller deliverable, or defer to a point release (v2.0.1).

### 第 4 步：Plan Release Candidate Process
...
**预期结果：** RC process documented with tagging convention, distribution method, testing checklist, and escalation criteria.

**失败处理：** If the RC process is skipped (pressure to release), document the risk. Untested releases have higher rollback probability.

### 第 5 步：Define Go/No-Go Checklist
...
**预期结果：** Go/no-go checklist with clear pass/fail criteria and decision rules.

**失败处理：** If the go/no-go meeting results in no-go, identify the blocking items, assign owners, set a new target date (typically 1-2 weeks later), and update the release plan.

### 第 6 步：Document Rollback Plan
...
**预期结果：** Rollback plan documented with triggers, procedure, communication template, and point release policy. Complete RELEASE-PLAN.md written.

**失败处理：** If rollback is not feasible (e.g., database migration already applied), document the forward-fix procedure instead. Every release should have a recovery path.
