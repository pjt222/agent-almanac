...
- **必需**: Project root directory containing a version file (DESCRIPTION, package.json, Cargo.toml, pyproject.toml, or VERSION)
- **必需**: Git history since the last release (tag or commit)
- **可选**: Commit convention in use (Conventional Commits, free-form)
- **可选**: Pre-release label to apply (alpha, beta, rc)
- **可选**: Previous version if not readable from files

## 步骤

### 第 1 步：Read Current Version
...
**预期结果：** Current version identified as `MAJOR.MINOR.PATCH[-PRERELEASE]`.

**失败处理：** If no version file is found, check for a VERSION file or git tags (`git describe --tags --abbrev=0`). If no version exists at all, start at `0.1.0` for initial development or `1.0.0` if the project has a stable public API.

### 第 2 步：Analyze Changes Since Last Release
...
**预期结果：** A list of commits with messages that can be classified by change type.

**失败处理：** If git history is unavailable or tags are missing, ask the developer to describe the changes manually. Classify based on their description.

### 第 3 步：Classify Changes
...
**预期结果：** Each change classified as breaking/feature/fix, and the overall bump level determined.

**失败处理：** If changes are ambiguous, err on the side of a higher bump. A conservative major bump is better than a minor bump that breaks downstream code.

### 第 4 步：Compute New Version
...
**预期结果：** New version number computed following SemVer rules.

**失败处理：** If the current version is malformed or non-SemVer, normalize it first. For example, `1.2` becomes `1.2.0`.

### 第 5 步：Update Version Files
...
**预期结果：** All version files updated consistently to the new version number.

**失败处理：** If a file update fails, revert all changes to maintain consistency. Never leave version files in a partially updated state.

### 第 6 步：Create Version Tag
...
**预期结果：** Git tag created matching the new version.

**失败处理：** If the tag already exists, the version was not properly bumped. Check for duplicate tags with `git tag -l "v1.3*"` and resolve before proceeding.
