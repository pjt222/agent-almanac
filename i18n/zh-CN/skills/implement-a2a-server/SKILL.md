...
- **必需**: Agent Card (JSON) defining the agent's skills and capabilities
- **必需**: Implementation language (TypeScript/Node.js or Python)
- **必需**: Task execution logic for each skill defined in the Agent Card
- **可选**: Push notification webhook support (`true` or `false`)
- **可选**: Persistent task store (in-memory, Redis, PostgreSQL)
- **可选**: Authentication middleware matching the Agent Card's auth scheme
- **可选**: Maximum concurrent tasks limit

## 步骤

### 第 1 步：Set Up Project with JSON-RPC 2.0 Handler
...
**预期结果：** An HTTP server that accepts JSON-RPC 2.0 requests and serves the Agent Card.

**失败处理：** If JSON-RPC parsing fails, validate that the request body has `jsonrpc`, `method`, and `id` fields. Return `-32700` (Parse error) for malformed JSON and `-32600` (Invalid Request) for missing required fields.

### 第 2 步：Implement Task State Machine
...
**预期结果：** A task store that enforces valid state transitions and maintains history.

**失败处理：** If an invalid state transition is attempted (e.g., `completed` to `working`), return a JSON-RPC error with code `-32002` and a descriptive message. Never silently ignore invalid transitions.

### 第 3 步：Add tasks/send and tasks/get Methods
...
**预期结果：** Working `tasks/send`, `tasks/get`, and `tasks/cancel` methods that correctly manage task lifecycle.

**失败处理：** If skill matching fails, return the task in `failed` state with a descriptive message. If the task store is full, return `-32003` (resource exhausted).

### 第 4 步：Implement SSE Streaming for tasks/sendSubscribe
...
**预期结果：** SSE streaming that sends real-time status and artifact events as the task progresses.

**失败处理：** If SSE connection drops, the client should be able to reconnect and use `tasks/get` to retrieve the current state. Ensure the task store does not depend on active SSE connections.

### 第 5 步：Add Push Notification Webhook Support
...
**预期结果：** Webhook registration and delivery with retry logic.

**失败处理：** Push notification failures must never affect task execution. Log errors and continue. If the webhook URL is persistently unreachable, remove the subscription after max retries.

### 第 6 步：Integrate with Agent Card for Discovery
...
**预期结果：** A running A2A server that serves its Agent Card, accepts tasks, and manages their full lifecycle.

**失败处理：** If the Agent Card capabilities do not match the implementation, the startup validation from 6.1 will catch the mismatch. Fix the implementation or update the Agent Card to match.
