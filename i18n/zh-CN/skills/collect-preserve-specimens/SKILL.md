...
- **必需**: Legal authorization to collect at the intended site (permits, landowner consent)
- **必需**: Collection equipment appropriate to the target taxa
- **必需**: Preservation materials (pins, ethanol, or both)
- **必需**: Labeling materials (archival paper, fine-point pen or printer)
- **可选**: Spreading boards for Lepidoptera and Odonata
- **可选**: Relaxing chamber for rehydrating dried specimens
- **可选**: Dissecting microscope for sorting and preparation
- **可选**: Database or catalog system for specimen records
- **可选**: Unit trays, drawers, and storage cabinets

## 步骤

### 第 1 步：Verify Legal Requirements
...
**预期结果：** All required permits are obtained, protected species lists have been checked, and the collector has a clear understanding of what may and may not be collected at the site.

**失败处理：** If permits cannot be obtained, do not collect. Photograph specimens in situ and use citizen science platforms for identification. If a collected specimen turns out to be a protected species, consult the relevant wildlife authority immediately. Do not discard the specimen — accidental collection should be reported, not concealed.

### 第 2 步：Select Collection Method
...
**预期结果：** One or more collection methods selected based on target taxa and habitat, with equipment assembled and ready before entering the field.

**失败处理：** If the intended method is not yielding specimens (e.g., sweep netting in heavy rain), switch to an alternative method. Light trapping requires specific weather conditions — warm, humid, still, moonless nights are optimal. If conditions are poor, reschedule rather than running traps inefficiently.

### 第 3 步：Dispatch Specimens Humanely
...
**预期结果：** Specimens dispatched quickly (within minutes) with minimal damage to morphological features. Lepidoptera kept separate to prevent scale loss from contact with other specimens.

**失败处理：** If ethyl acetate is unavailable, freezing is the best alternative for most taxa. In the field without either, place specimens in individual vials or envelopes (for Lepidoptera, use glassine envelopes with wings folded) and freeze upon return. Do not leave live insects in a sealed container without a killing agent — they will damage themselves.

### 第 4 步：Pin Specimens (Dry Preservation)
...
### 第 5 步：Preserve in Ethanol (Wet Preservation)
...
**预期结果：** Soft-bodied specimens preserved in 70-80% ethanol (or 95%+ for DNA work) in labeled vials with sufficient fluid volume.

**失败处理：** If ethanol is unavailable in the field, specimens can be temporarily stored in high-proof clear spirits (vodka, Everclear) as an emergency measure. Transfer to laboratory-grade ethanol as soon as possible. If specimens have been stored too long in weak preservative and show decomposition, they may still be identifiable but will be unsuitable for molecular work.

### 第 6 步：Label Every Specimen
...
**预期结果：** Every specimen carries at minimum a locality label with country, locality, coordinates, date, and collector. Wet specimens have the same data on an internal label written in pencil or laser-printed.

**失败处理：** If GPS coordinates were not recorded in the field, estimate them from a map using the locality description. If the date is uncertain, record the best estimate and mark it with a question mark. A specimen with approximate data is far more valuable than one with no label at all. Never move a label from one specimen to another.

### 第 7 步：Store and Protect the Collection
...
**预期结果：** All specimens cataloged in a database with unique identifiers, taxonomy, locality, date, collector, and storage location. Collection is on a regular inspection and maintenance schedule.

**失败处理：** If a full database is not feasible, maintain at minimum a handwritten catalog or spreadsheet with catalog numbers corresponding to specimens. The catalog number links the specimen to its data — without it, the specimen and its data become disconnected if labels are lost. Even a simple numbered list is better than no catalog at all.
...
- **Ethanol concentration too low for DNA work**: Specimens for molecular analysis require 95%+ ethanol with a change after 24 hours. Standard 70% preserves morphology but degrades DNA
- **Forgetting internal labels**: External vial labels can detach. Always place a pencil or laser-printed label inside with the specimen
- **Neglecting pest management**: Dermestid beetles can destroy a drawer of specimens in weeks. Regular pest monitoring is not optional
## 相关技能
- `identify-insect` — morphological identification using dichotomous keys and wing venation
- `document-insect-sighting` — photographic documentation that may supplement or replace collecting
- `observe-insect-behavior` — behavioral observation protocols for live insects
- `survey-insect-population` — systematic population surveys requiring specimen collection