...
- **必需**: Kubernetes manifests for your application (deployment, service, etc.)
- **必需**: Application name and version
- **必需**: List of configurable parameters (image tag, replicas, resources, etc.)
- **可选**: Dependencies on other Helm charts (databases, message queues)
- **可选**: Pre/post-install hooks for migrations or setup
- **可选**: Chart repository URL for publishing
- **可选**: Values for different environments

## 步骤

> See [Extended Examples](references/EXAMPLES.md) for complete template files, values structures, and hooks.

### 第 1 步：Initialize Chart Structure and Metadata
...
**预期结果：** Chart directory structure created with all required files. Chart.yaml contains complete metadata. Dependencies listed if applicable. Chart validates: `helm lint my-app`.
...
### 第 2 步：Design values.yaml Structure
...
**预期结果：** values.yaml organized logically with sections. All values documented with comments. Sensible defaults that work out-of-box. Schema validates value types. No hardcoded environment-specific values.
...
### 第 3 步：Create Template Files with Go Templating
...
**预期结果：** Templates generate valid Kubernetes YAML. Conditionals work correctly (if/with). Helper functions produce expected output. Resources properly labeled and named. No hardcoded values in templates.
...
### 第 4 步：Add Hooks for Pre/Post-Install Actions
...
**预期结果：** Hooks execute in correct order (weights determine sequence). Pre-install migration completes before deployment. Test hook validates deployment. Pre-delete hook runs cleanup. NOTES.txt provides helpful post-install information.
...
**预期结果：** Chart passes all lint checks. Template rendering produces valid Kubernetes YAML. Tests pass successfully. Chart packages without errors. Different values files work for each environment. Installation succeeds without warnings.
...
### 第 6 步：Publish to Chart Repository
...
**预期结果：** Chart published to repository successfully. Chart discoverable via `helm search`. Installation works from repository. Versioning follows SemVer.
