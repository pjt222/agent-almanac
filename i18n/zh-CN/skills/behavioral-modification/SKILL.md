...
- **必需**: A specific unwanted behavior to address (not "the dog is bad" but "the dog lunges at other dogs on leash")
- **必需**: The dog's threshold distance or trigger level (how close/intense before the behavior starts)
- **可选**: History of the behavior (when it started, what triggers it, what makes it worse)
- **可选**: High-value treats that the dog will eat even when mildly stressed
- **可选**: Veterinary clearance (rule out pain or medical causes for behavior changes)

## 步骤

### 第 1 步：Identify and Define the Behavior
...
**预期结果：** A precise behavior definition with identified trigger, threshold distance, and current consequence pattern.

**失败处理：** If the behavior seems to have no consistent trigger, keep a log for one week: date, time, context, behavior, consequence. Patterns often emerge that are not obvious in the moment.

### 第 2 步：Choose the Intervention Strategy
...
**预期结果：** A specific strategy selected for the identified behavior.

**失败处理：** If the behavior is severe (biting with contact, extreme panic, self-harm), refer to a certified applied animal behaviorist (CAAB) or veterinary behaviorist (DACVB). This skill covers moderate behavioral issues, not clinical cases.

### 第 3 步：Execute Desensitization and Counter-Conditioning
...
**预期结果：** Over weeks, the dog's threshold distance decreases and emotional response to the trigger shifts from fear/aggression to neutral or positive.

**失败处理：** If no progress after 3-4 weeks of consistent sessions, reassess: (1) are you working below threshold? (2) are the treats high-value enough? (3) is the trigger exposure too frequent outside of training (flooding undoes DS/CC)? (4) consider consulting a professional.

### 第 4 步：Manage the Environment
...
**预期结果：** The unwanted behavior is not being practiced outside of controlled training sessions.

**失败处理：** If management is impossible (e.g., cannot avoid all dog encounters), reduce training criteria to match reality. Some environmental exposure is unavoidable; ensure training sessions provide a strong enough counter-experience.
