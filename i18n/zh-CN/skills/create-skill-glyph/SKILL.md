...
- **必需**: Skill ID (e.g., `create-skill-glyph`) and domain (e.g., `design`)
- **必需**: Visual concept — what the glyph should depict
- **可选**: Reference glyph to study for complexity level
- **可选**: Custom `--glow-sigma` value (default: 4)

## 步骤

### 第 1 步：Concept — Design the Visual Metaphor
...
**预期结果：** A clear mental sketch of the shape with 2-6 planned layers.

**失败处理：** If the concept is too abstract, fall back to a related concrete object. Review existing glyphs in the same domain for inspiration.

### 第 2 步：Compose — Write the Glyph Function
...
**预期结果：** A working R function that returns a list of 2-6 ggplot2 layers.

**失败处理：** If `ggforce::geom_circle` causes errors, ensure ggforce is installed. If coordinates are off, remember the canvas is 100x100 with (0,0) at bottom-left. Test the function interactively:
```r
...
### 第 3 步：Register — Map Skill to Glyph
...
**预期结果：** The `SKILL_GLYPHS` list contains the new mapping.

**失败处理：** If the build later reports "No glyph mapped for skill", double-check that the skill ID exactly matches the one in the manifest and registry.

### 第 4 步：Manifest — Add Icon Entry
...
**预期结果：** Valid JSON with the new entry placed among its domain siblings.

**失败处理：** Validate JSON syntax. Common mistakes: trailing comma after last array element, missing quotes.

### 第 5 步：Render — Generate the Icon
...
**预期结果：** The log shows `OK: <domain>/<skill-id> (seed=XXXXX, XX.XKB)` and the WebP file exists.
...
### 第 6 步：Verify — Visual Inspection
...
**预期结果：** A clear, recognizable pictogram with even neon glow on transparent background.
...
### 第 7 步：Iterate — Refine if Needed
...
**预期结果：** The final icon passes all verification checks from Step 6.

**失败处理：** If after 3+ iterations the glyph still doesn't read well, consider using a completely different visual metaphor (return to Step 1).
