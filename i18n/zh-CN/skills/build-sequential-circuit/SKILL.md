...
- **必需**: Behavioral specification -- one of: state diagram, state table, timing diagram, regular expression to detect, or verbal description of the desired sequential behavior
- **必需**: Clock characteristics -- edge-triggered (rising/falling) or level-sensitive; single clock or multi-phase
- **可选**: Flip-flop type preference (D, JK, T, or SR)
- **可选**: Reset type -- synchronous, asynchronous, or none
- **可选**: Maximum state count or bit width constraint
- **可选**: Timing constraints (setup time, hold time, maximum clock frequency)

## 步骤

### 第 1 步：Identify Memory and State Requirements
...
**预期结果：** A complete state inventory with encoding chosen, flip-flop type selected, and the machine classified as Mealy or Moore.

**失败处理：** If the state count is unclear from the specification, enumerate states by tracing through all possible input sequences up to the memory depth of the circuit. If the count exceeds practical limits (more than 16 states for manual design), consider decomposing into smaller interacting FSMs.

### 第 2 步：Construct State Diagram and Transition Table
...
**预期结果：** A complete state transition table covering every present-state/input combination, with all states reachable from the initial state.

**失败处理：** If the transition table has missing entries, the specification is incomplete. Return to the requirements and resolve the ambiguity. If unreachable states exist, either add transitions to reach them or remove them and reduce the state encoding.

### 第 3 步：Derive Excitation Equations
...
**预期结果：** Minimized excitation equations for each flip-flop and output equations for each primary output, with all don't-cares exploited.

**失败处理：** If the excitation equations seem overly complex, reconsider the state encoding. A different encoding (e.g., switching from binary to one-hot, or reassigning state codes) can dramatically simplify the combinational logic. Try at least two encodings and compare literal counts.

### 第 4 步：Implement at Gate Level
...
**预期结果：** A complete gate-level netlist with flip-flops, excitation logic, output logic, clock distribution, and reset mechanism, where every signal has exactly one driver.

**失败处理：** If the implementation has feedback outside of the flip-flops, a combinational loop has been introduced. All feedback in a synchronous sequential circuit must pass through a flip-flop. Trace the offending path and reroute it through a register.

### 第 5 步：Verify via Timing Diagram and State Sequence Simulation
...
**预期结果：** Every cycle in the timing diagram matches the state transition table, outputs are correct for every cycle, and no timing violations are present.

**失败处理：** If a state transition is wrong, trace the excitation logic for that specific present-state and input combination. If outputs are wrong but transitions are correct, the error is in the output logic. If the circuit enters an unintended state, check for incomplete reset or missing transitions from unused state codes.
