...
- **必需**: Source files to annotate
- **必需**: Annotation plan or knowledge of the workflow steps
- **可选**: Style preference: single-line or multiline (default: single-line)
- **可选**: Whether to use `put_generate()` for skeleton generation (default: yes)

## 步骤

### 第 1 步：Determine Comment Prefix
...
**失败处理：** If the extension is not recognized, the file language may not be supported. Check `get_supported_extensions()` for the full list. For unsupported languages, use `#` as a conventional default.

### 第 2 步：Generate Annotation Skeletons
...
**预期结果：** One or more annotation comment lines per source file, pre-filled with detected function names and I/O.

**失败处理：** If no suggestions are generated, the file may not contain recognizable I/O patterns. Write annotations manually based on your understanding of the code.

### 第 3 步：Refine Annotations
...
**预期结果：** Annotations refined with accurate IDs, labels, and I/O fields that reflect actual data flow.

**失败处理：** If unsure about I/O, use `.internal` extension for in-memory intermediates and explicit file names for persisted data.

### 第 4 步：Insert Annotations into Files
...
**预期结果：** Annotations inserted at appropriate locations in each source file.

**失败处理：** If annotations break syntax highlighting in the editor, ensure the comment prefix is correct for the language. PUT annotations are standard comments and should not affect code execution.

### 第 5 步：Validate Annotations
...
**预期结果：** All annotations parse without errors. The diagram shows a connected workflow. `put_merge()` fills in any gaps from auto-detection.

**失败处理：** Common validation issues:
