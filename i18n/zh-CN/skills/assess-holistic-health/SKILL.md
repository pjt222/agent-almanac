...
- **必需**: Current symptoms or health concerns (physical, mental, emotional)
- **必需**: Responses to temperament assessment questions (constitution, habits, preferences)
- **可选**: Age, sex, and general health history (for context)
- **可选**: Current season (for seasonal adjustment recommendations)
- **可选**: Existing dietary restrictions or preferences
- **可选**: Known temperament from previous assessment

## 步骤

### 第 1 步：Temperament Assessment
...
**预期结果：** Dominant and secondary temperament identified, with score profile showing primary constitutional tendencies.

**失败处理：** If all scores are within 2-3 points of each other, the person has a balanced temperament (unusual in Hildegard's system). Proceed with general recommendations avoiding extremes (no very hot or very cold foods).

### 第 2 步：Elemental Balance Assessment
...
**预期结果：** Identification of 1-2 elemental imbalances (e.g., "Excess water with deficient fire" = cold/damp pattern with low vitality).

**失败处理：** If symptom pattern is unclear or contradictory, default to the opposite qualities of the dominant temperament. Example: Choleric (hot/dry) feeling unwell likely has excess fire → recommend cooling/moistening.

### 第 3 步：Dietary Recommendations
...
**预期结果：** Specific food recommendations tailored to temperament and current imbalance, with clear "favor/moderate/avoid" categories.

**失败处理：** If dietary restrictions prevent following recommendations (e.g., vegetarian unable to eat lamb), substitute within the same elemental category (warming/cooling/moistening/drying). Consult the plant-based options within each category.

### 第 4 步：Lifestyle and Seasonal Adjustments
...
**预期结果：** 3-5 specific lifestyle recommendations that address the identified imbalance and align with Hildegard's holistic view (body, mind, spirit, nature).

**失败处理：** If recommendations feel overwhelming, prioritize ONE dietary change and ONE lifestyle change to begin. Full rebalancing takes weeks to months — start small.

### 第 5 步：Integration and Monitoring
...
**预期结果：** User has clear timeline for change implementation, monitoring practices, and criteria for reassessment.

**失败处理：** If user reports feeling worse after 1-2 weeks, reassess for overcorrection. Example: Adding too many warming foods to a choleric type could cause excess fire. Dial back to neutral foods temporarily.
