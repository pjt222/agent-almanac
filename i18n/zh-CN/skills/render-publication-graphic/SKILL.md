...
**预期结果：** Clear understanding of target requirements
**失败处理：** Contact publisher for specific guidelines, use conservative defaults
...
**预期结果：** Graphics rendered at correct resolution for print quality
**失败处理：** Verify DPI metadata saved correctly, check file size appropriate
...
**预期结果：** Color space matches publication requirements
**失败处理：** Verify color profile embedded, test print preview
...
**预期结果：** Text readable at publication size, fonts embedded properly
**失败处理：** Increase font sizes, check font licensing, convert text to outlines
...
**预期结果：** Appropriate format for publication channel
**失败处理：** Check publisher requirements, provide multiple formats
...
**预期结果：** Web-optimized images under 500KB, responsive sizes generated
**失败处理：** Reduce quality, resize further, consider WebP format
...
**预期结果：** Metadata embedded and retrievable
**失败处理：** Check format supports metadata (PNG, TIFF, PDF yes; JPEG limited)
