...
- **必需**: What the person needs to teach or explain (topic, concept, system, process)
- **必需**: Who the audience is (expertise level, context, relationship to the person)
- **可选**: Format of delivery (presentation, documentation, one-on-one mentoring, workshop)
- **可选**: Time constraints (5-minute explanation, 30-minute talk, written document)
- **可选**: Previous teaching attempts and what did not work
- **可选**: The person's own comfort level with the topic (deep expert vs. recent learner)

## 步骤

### 第 1 步：Assess — Understand the Teaching Challenge
...
**预期结果：** A clear picture of the teaching challenge: what, to whom, in what format, with what constraints, and where the person feels least confident.

**失败处理：** If the person cannot articulate their audience, help them create a persona: "Imagine one specific person who will hear this. What do they know? What do they care about?" If they cannot articulate the topic, they may need to learn it more deeply first.

### 第 2 步：Structure — Organize Content for Clarity
...
**预期结果：** A structured outline where every element serves the core message. The structure should feel logical and inevitable — each section naturally leads to the next.

**失败处理：** If the structure keeps growing, the scope is too broad — help them cut. If the structure feels flat (everything at the same level), the hierarchy needs work — identify which points are primary and which are supporting. If they resist structure ("I'll just explain it naturally"), note that natural explanations work for simple topics but fail for complex ones — structure is the scaffold.

### 第 3 步：Practice — Rehearse the Explanation
...
**预期结果：** Targeted feedback that improves the explanation measurably. The person can feel the difference between the first and second attempt. Feedback is framed constructively — what to do, not just what to avoid.

**失败处理：** If the person is defensive about feedback, reframe from "this was unclear" to "the audience might not follow here — how could we make it even clearer?" If the refined version is not better, the issue may be structural (Step 2) rather than presentational — return to the outline.

### 第 5 步：Deliver — Support During Teaching
...
**预期结果：** The person feels prepared and supported. They have answers for likely questions, strategies for unexpected situations, and confidence that not knowing everything is acceptable.

**失败处理：** If anxiety is the primary blocker, address it directly: preparation reduces anxiety, and acknowledging nervousness to the audience often creates connection. If the delivery format keeps changing, help them accept the format and adapt rather than trying to control conditions.

### 第 6 步：Reflect — Analyze What Worked
...
**预期结果：** The person gains concrete insight about their teaching effectiveness — not vague feelings but specific observations about what worked and why. They leave with one actionable improvement for next time.

**失败处理：** If they only see negatives, redirect to specific moments that worked. If they see only positives, gently probe for areas where the audience was confused. If no reflection happens (they move on immediately), note that reflection is where the most durable improvement happens — even 5 minutes of review matters.
