...
- **必需**: A focal insect or insect aggregation to observe
- **必需**: A timing device (watch, phone, or stopwatch)
- **必需**: Recording method (notebook, voice recorder, or data entry device)
- **可选**: Hand lens (10x) for close behavioral observation
- **可选**: Binoculars for observing insects at distance (e.g., dragonflies on patrol)
- **可选**: Camera for video documentation of behavioral events
- **可选**: Thermometer, hygrometer, or weather station for environmental data
- **可选**: Pre-printed data sheets or ethogram templates

## 步骤

### 第 1 步：Choose a Sampling Protocol
...
**预期结果：** A sampling protocol selected and justified based on the research question, target taxon, and field conditions. Recording interval or focal bout duration defined before observation begins.

**失败处理：** If the target insect is too mobile for focal animal sampling (e.g., a fast-flying dragonfly), switch to all-occurrences sampling focused on specific events (territorial chases, perch returns). If you cannot distinguish individuals for focal sampling, use scan sampling on the group. Adapt the protocol to what is feasible rather than abandoning observation.

### 第 2 步：Define the Ethogram
...
**预期结果：** A complete ethogram defined for the target taxon before observation begins. Categories should be mutually exclusive (any behavior fits in exactly one category) and exhaustive (every observed behavior can be classified).

**失败处理：** If an unexpected behavior occurs that does not fit the ethogram, record it verbatim (e.g., "rapid wing vibration while stationary, not matching any defined category") and add a new category in the post-observation ethogram revision. Do not force novel behaviors into ill-fitting categories.

### 第 3 步：Record Behavioral Events with Timestamps
...
**预期结果：** All observed interactions recorded with initiator, partner identity (to lowest taxonomic level possible), behaviors of both parties, and outcome.

**失败处理：** If interactions happen too rapidly to record in full (e.g., a swarm of competing males), focus on the focal individual's behavior and note "multiple simultaneous interactions — details approximate." If partner identity is unknown, describe it (e.g., "small black hymenopteran, approximately 8mm").

### 第 5 步：Record Environmental Context
...
**预期结果：** Environmental covariates recorded at the start and end of each observation session, with intermediate readings for sessions longer than 1 hour.

**失败处理：** If instrumentation is unavailable, estimate temperature ("warm, approximately 25C"), humidity ("dry" or "humid"), and wind from sensory cues. Approximate environmental data is far more useful than no environmental data. At minimum, record time of day, cloud cover, and estimated temperature.

### 第 6 步：Summarize Observations
...
**预期结果：** A structured summary including time budget or behavioral frequencies (depending on sampling protocol), interaction summary, observed patterns, and explicit acknowledgment of limitations.

**失败处理：** If the observation session was too short for meaningful time budgets (less than 10 minutes of continuous data), report raw event counts rather than proportions. Note the short duration as a limitation. Even brief observations contribute to understanding if they are honestly reported — a 5-minute observation documenting a rare behavior (e.g., parasitoid oviposition) can be more valuable than hours of resting behavior.
