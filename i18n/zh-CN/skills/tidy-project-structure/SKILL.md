...
**预期结果：** List of files/directories violating conventions saved to `structure_audit.txt`

**失败处理：** If no conventions documented, use language-standard defaults

### 第 2 步：Move Misplaced Files
...
**预期结果：** All files in conventional locations; git history preserved via `git mv`

**失败处理：** If moving breaks imports, update import paths or escalate

### 第 3 步：Check README Freshness
...
**预期结果：** List of stale READMEs in `readme_freshness.txt` with specific issues

**失败处理：** If markdown-link-check unavailable, manually review external links

### 第 4 步：Update Stale READMEs
...
**预期结果：** All READMEs updated; examples verified to run

**失败处理：** If example code cannot be verified, mark with warning comment

### 第 5 步：Review Config Files
...
**预期结果：** Config drift documented in `config_review.txt`; secrets flagged for escalation

**失败处理：** If diff shows major divergence, escalate to devops-engineer

### 第 6 步：Archive Deprecated Files
...
**预期结果：** Deprecated files archived; `ARCHIVE_LOG.md` updated

**失败处理：** If uncertain whether file is deprecated, leave in place and document in report

### 第 7 步：Verify Naming Conventions
...
**预期结果：** All files follow naming conventions or exceptions documented

**失败处理：** If renaming breaks imports, update references or escalate

### 第 8 步：Generate Tidying Report
...
**预期结果：** Report saved to `TIDYING_REPORT.md`
