...
- **必需**: Device type (electromagnet, motor, generator, or transformer)
- **必需**: Performance requirements (field strength, force, torque, voltage ratio, power, efficiency target)
- **必需**: Operating conditions (supply voltage and current, frequency, duty cycle, ambient temperature)
- **可选**: Preferred core material (silicon steel, ferrite, powdered iron, air core) with B-H data
- **可选**: Size and weight constraints
- **可选**: Cost or manufacturing constraints

## 步骤

### 第 1 步：Specify Device Requirements and Operating Conditions
...
**预期结果：** A justified topology selection with clear reasoning tied to the requirements from Step 1, including acknowledged limitations.

**失败处理：** If no standard topology meets all requirements, consider a hybrid design (e.g., Halbach array for higher field with less material) or relax a secondary constraint. Document the tradeoff.

### 第 3 步：Calculate Design Parameters
...
**预期结果：** Numerical values for all physical dimensions and electrical parameters, derived from electromagnetic equations with units checked at each step.

**失败处理：** If the required turns do not fit in the available winding space, either increase the core size (larger window area), use finer wire (higher current density, but more heating), or reduce the performance target. If the core operates above B_max, increase the core cross-section or add turns (to reduce the flux for the same performance via a larger NI product with a larger gap).

### 第 4 步：Analyze Losses and Efficiency
...
**预期结果：** All requirements met with documented margins, thermal feasibility confirmed, and the most sensitive design parameter identified.

**失败处理：** If a requirement is not met, iterate by adjusting the topology (Step 2), design parameters (Step 3), or loss mitigation strategy (Step 4). If the design is thermally infeasible, consider: reducing the duty cycle, increasing the size (more surface area for cooling), switching to a higher temperature insulation class, or adding active cooling. Document each iteration.
