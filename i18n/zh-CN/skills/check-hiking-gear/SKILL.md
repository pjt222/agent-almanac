...
- **必需**: Hike duration (day hike, overnight, multi-day)
- **必需**: Season and expected temperature range
- **必需**: Trail difficulty (SAC T1-T6 or descriptive)
- **可选**: Maximum elevation and expected conditions (snow, rain, heat)
- **可选**: Group size (for distributing shared gear)
- **可选**: Target pack weight or weight limit
- **可选**: Special requirements (via ferrata gear, glacier equipment, photography)

## 步骤

### 第 1 步：Assess Conditions
...
**预期结果：** A clear hike profile with all condition factors assessed. This profile drives the checklist in Step 2.

**失败处理：** If conditions are uncertain (e.g., shoulder season with unpredictable weather), plan for the worse case. It is always better to carry a rain jacket you don't use than to be soaked without one.

### 第 2 步：Generate Base Checklist by Category
...
**预期结果：** A complete checklist with all ten essentials, appropriate clothing layers, and profile-specific additions. Every item is relevant to the assessed conditions.

**失败处理：** If the list seems excessive for a short easy hike, verify that only the base ten essentials are included for SUMMER-DAY profiles. If the list seems too light for alpine conditions, cross-reference with the Alpine profile add-ons.

### 第 3 步：Optimize Weight
...
**预期结果：** A weight-optimized checklist where every item serves a clear purpose. Total pack weight is within the target range for the hike profile. Shared gear is assigned to specific group members.

**失败处理：** If pack weight exceeds the target by more than 20%, reconsider whether the hike profile is appropriate. A heavily loaded pack on a long day dramatically increases fatigue and injury risk. Either reduce gear (accept more risk) or choose an easier/shorter route.

### 第 4 步：Verify Completeness Against Conditions
...
**预期结果：** All checks pass. The hiker can confidently state what every item in the pack is for and would notice if any item were missing.

**失败处理：** If any essential check fails, resolve it before departure. The most dangerous failures are: no navigation backup (phone dies), insufficient water capacity, and missing insulation layer (hypothermia risk even in summer above treeline).
