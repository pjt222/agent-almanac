...
- **必需**: Category to consult (plants, stones, animals, fish, birds, elements, trees, reptiles, metals)
- **必需**: Specific item or property inquiry (e.g., "emerald", "fennel", "properties of fire element")
- **可选**: Application context (medicinal, symbolic, liturgical, practical)
- **可选**: Related temperament or ailment (to guide property interpretation)
- **可选**: Cross-reference request (e.g., "plants and stones for cold temperament")

## 步骤

### 第 1 步：Identify the Category in Physica
...
**预期结果：** Correct book/category identified for the inquiry (e.g., "emerald" → Book IV Stones; "fennel" → Book I Plants).

**失败处理：** If uncertain which category, search multiple. Example: "willow" could be Book I (plant) or Book III (tree) — Hildegard lists it in Book III as a tree with bark and wood properties.

### 第 2 步：Extract Properties and Applications
...
**预期结果：** Properties extracted with temperature, moisture, medicinal use, and symbolic associations clearly noted.

**失败处理：** If *Physica* entry is brief or unclear, supplement with known temperamental logic. Example: If Hildegard says only "it is warm," infer that it treats cold conditions and avoid in hot conditions.

### 第 3 步：Cross-Reference Between Categories (Optional)
...
**预期结果：** Related items identified across 2-3 categories that share temperament, medicinal use, or symbolic meaning. Synergistic use suggested where appropriate.

**失败处理：** If cross-references are unclear, focus on single-category lookup. Cross-referencing is enrichment, not essential for basic consultation.

### 第 4 步：Application Guidance
...
**预期结果：** Clear, actionable guidance for using *Physica* knowledge in context (medicinal, spiritual, seasonal, scholarly). User knows what to do with the information.

**失败处理：** If application is unclear, provide the raw *Physica* entry text and let user determine application. Hildegard's entries are often self-explanatory.

### 第 5 步：Contextualize within Hildegard's Holistic System
...
**预期结果：** User understands *Physica* as one component of Hildegard's holistic system. Cross-references to temperament, viriditas, and liturgical context provided where relevant.

**失败处理：** If integration feels complex, focus on immediate practical use (Step 4) and defer holistic integration for advanced study.
