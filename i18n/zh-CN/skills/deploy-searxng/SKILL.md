...
- **必需**: Server or machine with Docker installed
- **可选**: Domain name for public access
- **可选**: SSL certificate or Let's Encrypt setup
- **可选**: Custom engine preferences

## 步骤

### 第 1 步：Create Project Structure
...
### 第 2 步：Write Docker Compose File
...
### 第 3 步：Configure SearXNG Settings
...
### 第 4 步：Configure Nginx Frontend
...
**预期结果：** SearXNG responds on port 8080 through Nginx. Search queries return aggregated results.

**失败处理：** Check `docker compose logs searxng` for config errors. Verify `settings.yml` YAML syntax.

### 第 7 步：Add SSL (Production)
