...
- **必需**: Workflow data from `put()`, `put_auto()`, or `put_merge()`
- **可选**: Theme name (default: `"light"`; options: light, dark, auto, minimal, github, viridis, magma, plasma, cividis)
- **可选**: Output target: console, file path, clipboard, or raw string
- **可选**: Interactive features: `show_source_info`, `enable_clicks`

## 步骤

### 第 1 步：Extract Workflow Data
...
**预期结果：** A data frame with at least one row, containing `id`, `label`, and optionally `input`, `output`, `source_file`, `node_type` columns.

**失败处理：** If the data frame is empty, no annotations or patterns were found. Run `analyze-codebase-workflow` first, or check that annotations are syntactically valid with `put("./src/", validate = TRUE)`.

### 第 2 步：Select Theme and Options
...
**预期结果：** Theme names printed. Select one based on context.

**失败处理：** If a theme name is not recognized, `put_diagram()` falls back to `"light"`. Check spelling.

### 第 3 步：Custom Palette with `put_theme()` (Optional)
...
**预期结果：** Mermaid output with your custom classDef lines. Node shapes from `node_type` are preserved; only colors change. All node types use `stroke-width:2px` — override not currently supported via `put_theme()`.

**失败处理：** If the palette object is not a `putior_theme` class, `put_diagram()` raises a descriptive error. Ensure you pass the return value of `put_theme()`, not a raw list.
...
**预期结果：** Valid Mermaid code starting with `flowchart TD` (or `LR` depending on direction). Nodes are connected by arrows showing data flow.

**失败处理：** If the output is `flowchart TD` with no nodes, the workflow data frame is empty. If connections are missing, check that output filenames match input filenames across nodes.

### 第 5 步：Embed in Target Document
...
**预期结果：** Diagram renders correctly in the target format. GitHub renders mermaid code fences natively.

**失败处理：** If GitHub doesn't render the diagram, ensure the code fence uses exactly ` ```mermaid ` (no extra attributes). For Quarto, ensure the `knit_child()` approach is used since direct variable interpolation in `{mermaid}` chunks is not supported.
