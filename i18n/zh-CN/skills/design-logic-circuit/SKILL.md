...
- **必需**: Functional specification -- one of: truth table, Boolean expression, verbal description of input/output behavior, or a standard block name (e.g., "4-bit ripple-carry adder")
- **必需**: Target gate library -- unrestricted (AND/OR/NOT), NAND-only, NOR-only, or a specific standard cell library
- **可选**: Optimization goal -- minimize gate count, minimize propagation delay (critical path), or minimize fan-out
- **可选**: Maximum fan-in constraint (e.g., 2-input gates only)
- **可选**: Don't-care conditions for inputs that will never occur

## 步骤

### 第 1 步：Specify Circuit Function
...
**预期结果：** A complete, unambiguous specification where every legal input combination maps to exactly one output value.

**失败处理：** If the specification is ambiguous (e.g., missing cases, conflicting outputs for the same input), request clarification. Do not assume don't-care for unspecified inputs unless explicitly told to.

### 第 2 步：Derive Minimal Boolean Expression
...
**预期结果：** A minimal Boolean expression for each output, with shared sub-expressions identified for multi-output circuits.

**失败处理：** If the expressions appear non-minimal (more literals than expected for the function's complexity), re-run the K-map or Quine-McCluskey step from evaluate-boolean-expression. For functions with more than 6 variables, use Espresso or a similar heuristic minimizer.

### 第 3 步：Map to Gate-Level Schematic
...
**预期结果：** A complete gate-level netlist where every output can be traced back to primary inputs through a chain of gates, with no floating (unconnected) inputs or outputs.

**失败处理：** If the netlist has dangling wires or feedback loops (which are invalid in combinational circuits), recheck the mapping. Every signal must have exactly one driver and every gate input must connect to either a primary input or another gate's output.

### 第 4 步：Convert to Universal Gate Basis (Optional)
...
**预期结果：** A functionally equivalent circuit using only the target gate type, with redundant inversions eliminated via bubble pushing.

**失败处理：** If the converted circuit has more inversions than expected, re-examine the bubble-pushing step. A common mistake is forgetting that NAND and NOR are self-dual under complementation -- applying De Morgan consistently from outputs back to inputs avoids this.

### 第 5 步：Verify via Exhaustive Simulation
...
**预期结果：** All test vectors pass. The circuit is functionally correct and the critical path depth is documented.

**失败处理：** If any vector fails, trace the signal path for that input combination gate by gate to find the first gate producing an incorrect output. Common causes: a wire connected to the wrong gate input, a missing inversion, or an error in the NAND/NOR conversion.
