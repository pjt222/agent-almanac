...
- **必需**: Project root directory containing dependency/lock files
- **可选**: Ecosystem type if not auto-detectable (R, Node.js, Python, Rust)
- **可选**: Security-only mode flag (skip staleness, focus on CVEs)
- **可选**: Allowlist of dependencies to skip (known acceptable older versions)
- **可选**: Target date for compatibility (e.g., "must work with R 4.4.x")

## 步骤

### 第 1 步：Inventory All Dependencies
...
**预期结果：** Complete inventory of all direct and (optionally) transitive dependencies with pinned versions.

**失败处理：** If lock files are missing, the project has reproducibility issues. Note this as a finding and inventory from the manifest file (DESCRIPTION, package.json) using declared version constraints instead of pinned versions.

### 第 2 步：Check Latest Available Versions
...
**预期结果：** Latest version identified for each dependency with the gap magnitude (patch/minor/major).

**失败处理：** If a package registry is unreachable, note the dependency as "unable to check" and proceed with the rest. Do not block the entire audit on one unreachable registry.

### 第 3 步：Classify Staleness
...
**预期结果：** Every dependency classified by staleness with an overall health rating.

**失败处理：** If version comparison logic is ambiguous (non-SemVer versions, date-based versions), classify conservatively as "minor behind" and note the non-standard versioning.

### 第 4 步：Check for Security Vulnerabilities
...
**预期结果：** Security vulnerabilities identified with CVE, severity, affected version, and fix version.

**失败处理：** If no audit tool is available for the ecosystem, search GitHub Security Advisories manually for each dependency. Note that the audit is best-effort without tooling.

### 第 5 步：Plan Upgrade Path
...
**预期结果：** Prioritized upgrade plan with security fixes first, then EOL replacements, major upgrades, and minor/patch batches.

**失败处理：** If a dependency has no clear upgrade path (abandoned with no fork), document the risk and recommend: (1) vendoring the current version, (2) finding an alternative package, or (3) accepting the risk with monitoring.

### 第 6 步：Document Compatibility Risks
...
**预期结果：** Compatibility risks documented for each significant upgrade. Complete audit report written.

**失败处理：** If compatibility cannot be assessed without testing, recommend a branch-based upgrade approach: create a branch, apply the upgrade, run tests, and evaluate results before merging.
