...
- **必需**: Training dataset with features and labels
- **必需**: Validation dataset for objective evaluation
- **必需**: Model type(s) to optimize (e.g., XGBoost, LightGBM, neural network)
- **必需**: Optimization objective (metric to maximize/minimize)
- **必需**: Compute budget (time or number of trials)
- **可选**: Search space constraints (min/max values for hyperparameters)
- **可选**: Prior knowledge of good hyperparameter ranges

## 步骤

### 第 1 步：Install Dependencies and Set Up Environment
...
**预期结果：** Clean environment with required packages installed, no dependency conflicts.

**失败处理：** Use Python 3.8-3.11 (compatibility issues with 3.12+), if CUDA errors occur install CPU-only versions first, on M1/M2 Mac use conda instead of pip for scikit-learn.

### 第 2 步：Define Search Space and Objective (Optuna)
...
**预期结果：** Search space covers reasonable hyperparameter ranges, objective function runs without errors, pruning stops unpromising trials early.

**失败处理：** If trials crash, reduce search space (e.g., lower max n_estimators), verify data has no NaN/inf values, check memory usage (reduce batch size if OOM), ensure eval_metric matches task type.

### 第 3 步：Run Optimization with Advanced Samplers
...
**预期结果：** Optimization completes with 50-70% of trials pruned early, best parameters found, visualization plots generated showing convergence.

**失败处理：** If no pruning happens, verify objective reports intermediate values correctly, if optimization doesn't improve try different sampler (TPE → CmaES), if crashes with n_jobs>1 use n_jobs=1 for debugging.

### 第 4 步：Set Up Ray Tune for Distributed Optimization (Alternative)
...
**预期结果：** Ray Tune runs trials in parallel across CPUs/GPUs, ASHA scheduler stops bad trials early, best configuration found and logged.

**失败处理：** If Ray crashes, start with `ray.init(num_cpus=2, num_gpus=0)` for debugging, reduce concurrent trials if OOM, check that train function doesn't modify shared data, use `tune.report()` not `return` for metrics.

### 第 5 步：Track Experiments with MLflow
...
**预期结果：** All trials logged to MLflow with parameters and metrics, best model registered in MLflow registry, experiments viewable in MLflow UI.

**失败处理：** Start MLflow UI with `mlflow ui --backend-store-uri file:./automl/mlruns`, check write permissions to mlruns directory, if registration fails verify model registry is configured, ensure model artifact size < 2GB.

### 第 6 步：Deploy Best Model and Monitor Performance
...
**预期结果：** Model saved in production-ready format, configuration documented, inference script created for deployment.

**失败处理：** If model file too large (>100MB), consider model compression or feature selection, verify model loads correctly in fresh Python session, test inference script with sample data before deployment.
