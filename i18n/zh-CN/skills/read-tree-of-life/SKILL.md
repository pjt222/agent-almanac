...
- **必需**: A sephira, path, or structural element to explore (e.g., "Tiferet," "the path between Hod and Yesod," "the Pillar of Severity")
- **可选**: Tradition preference (classical Jewish Kabbalah, Hermetic Qabalah, comparative)
- **可选**: Depth level (overview, detailed study, contemplative exercise)
- **可选**: Current context or question the Tree should illuminate

## 步骤

### 第 1 步：Identify the Sephira or Path to Explore
...
**预期结果：** A clear identification of where on the Tree the exploration begins. The user can mentally locate the element within the overall structure.

**失败处理：** If the user's request is vague (e.g., "tell me about the Tree"), start with an overview of the three pillars and ten sephirot, then ask which area to explore in depth.

### 第 2 步：Study the Sephira's Attributes
...
**预期结果：** A complete attribute profile for the sephira. The user understands what the sephira represents, how it is addressed in practice, and what correspondences link it to other symbolic systems.

**失败处理：** If the user is overwhelmed by correspondences, present only name, translation, number, pillar, and one-sentence summary. Add detail incrementally on request.

### 第 3 步：Examine Connecting Paths
...
**预期结果：** The user sees the sephira in context — not isolated but connected to its neighbors through specific paths, each carrying its own symbolic weight.

**失败处理：** If path attributions are confusing, focus on the sephira's direct neighbors (which sephirot it connects to) without detailing individual path letters. Add letter attributions when the user is ready.

### 第 4 步：Locate Within the Four Worlds
...
**预期结果：** The user understands that the Tree is not flat but layered — the same structure repeats at four levels of reality, and the sephira under study has a specific "home world" with expressions at all levels.

**失败处理：** If the four-worlds framework is too abstract, simplify: Atzilut = divine, Briah = intellectual, Yetzirah = emotional, Assiah = physical. Ask which level the user wants to focus on.

### 第 5 步：Contemplative Exercise
...
**预期结果：** The user has moved from intellectual study to experiential engagement with the sephira. The contemplative exercise grounds abstract symbolism in personal reflection.

**失败处理：** If meditation feels forced or artificial, replace with journaling: write for 5 minutes about how the sephira's quality appears in your current life. Written reflection achieves similar integration through a different mode.
