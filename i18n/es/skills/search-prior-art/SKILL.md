...
- **Requerido**: Invention description (what it does, how it works, what problem it solves)
- **Requerido**: Search purpose (patentability, invalidity, FTO, defensive)
- **Requerido**: Critical date (filing date of the patent application, or invention date for prior art)
- **Opcional**: Known related patents or publications
- **Opcional**: Technology classification codes (IPC, CPC)
- **Opcional**: Key inventors or companies in the field

## Procedimiento

### Paso 1: Decompose the Invention into Searchable Elements
...
**Esperado:** A complete decomposition with search terms for each element. The novel combination is identified — this is what the search must either find (to invalidate) or confirm is absent (to support novelty).

**En caso de fallo:** If the invention is too abstract to decompose, ask for a more specific description. If the claims are unclear, focus on the broadest reasonable interpretation of each claim element.

### Paso 2: Search Patent Literature
...
**Esperado:** A classified list of patent references mapped to the invention's elements. X references (if found) are showstoppers for novelty. Y references are the building blocks for obviousness arguments.

**En caso de fallo:** If no relevant patent art is found, this doesn't mean the invention is novel — non-patent literature (Step 3) may contain the critical reference. Absence in one database doesn't mean absence everywhere.

### Paso 3: Search Non-Patent Literature
...
**Esperado:** Non-patent references that complement the patent search. Academic papers and open-source code are often the most powerful prior art because they tend to describe technical details more explicitly than patents.

**En caso de fallo:** If non-patent literature is sparse, the technology may be primarily developed in corporate R&D (patent-heavy). Shift emphasis to patent literature and focus on the combination-based obviousness argument.

### Paso 4: Analyze and Map Results
...
**Esperado:** A clear claim chart showing which elements are covered by which references, with an assessment of novelty and obviousness. Each mapping cites specific passages or figures in the references.

**En caso de fallo:** If the claim chart shows gaps (elements not found in any prior art), those gaps represent the potentially novel aspects. Focus follow-up searches on those specific gaps.

### Paso 5: Document and Deliver
...
**Esperado:** A complete, well-organized search report that directly supports the intended decision. References are accessible and analysis is traceable.

**En caso de fallo:** If the search is inconclusive (no strong X or Y references, but some relevant background), state the conclusion clearly: "No anticipatory art found; closest art addresses elements A and B but not C. Recommend filing with claims emphasizing element C." Inconclusive is a valid and useful result.
