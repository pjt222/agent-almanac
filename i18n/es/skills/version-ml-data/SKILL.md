...
- **Requerido**: Git repository for metadata tracking
- **Requerido**: DVC installation (`pip install dvc`)
- **Requerido**: Raw data files or directories to version
- **Opcional**: Remote storage backend (S3, Azure Blob, GCS, SSH, local)
- **Opcional**: Data processing scripts for pipeline automation
- **Opcional**: CI/CD integration for automated pipeline execution

## Procedimiento

### Paso 1: Initialize DVC in Git Repository
...
**Esperado:** `.dvc/` directory created with config files, `.dvcignore` file present, DVC files tracked by Git, large data files not in Git staging area.

**En caso de fallo:** Verify Git repository initialized (`git status`), check DVC installation (`dvc version`), ensure write permissions in project directory, check for conflicting `.dvc/` directory from previous setup, verify Python environment active.

### Paso 2: Configure Remote Storage Backend
...
**Esperado:** Remote storage configured and accessible, credentials stored securely in `.dvc/config.local` (git-ignored), test push/pull succeeds, remote storage shows uploaded cache files.

**En caso de fallo:** Verify cloud credentials (`aws s3 ls` or equivalent CLI), check bucket/container exists and is accessible, ensure IAM permissions for read/write, verify network connectivity to remote, check firewall rules, test SSH key authentication for SSH remotes, verify storage path has write permissions.

### Paso 3: Version Datasets with DVC
...
**Esperado:** `.dvc` metadata files created and committed to Git, original data files git-ignored automatically, `dvc push` uploads data to remote storage, `.dvc/cache` contains data hash, remote storage has cached data files.

**En caso de fallo:** Check DVC remote configured (`dvc remote list`), verify write permissions in data directory, ensure sufficient disk space for cache, check network connectivity for push, verify no special characters in file paths, check for large file warnings from Git.

### Paso 4: Build Reproducible Data Pipelines
...
**Esperado:** DVC pipeline executes in correct dependency order, only changed stages rerun, outputs cached efficiently, metrics tracked automatically, Git commits include `dvc.yaml` and `dvc.lock`.

**En caso de fallo:** Check script paths exist and are executable, verify dependencies specified correctly, ensure params.yaml keys match script usage, check for circular dependencies in pipeline, verify output paths writable, inspect script error messages in stderr, check Python environment has required packages.

### Paso 5: Share and Reproduce Data Versions
...
**Esperado:** `git clone` + `dvc pull` reproduces exact environment, data versions match across team, experiments isolated in branches, metrics comparable across versions.

**En caso de fallo:** Verify remote access configured correctly, check credentials for new team members, ensure all .dvc files committed to Git, verify `dvc.lock` tracked by Git (pins exact versions), check network bandwidth for large pulls, verify storage backend has all referenced cache files.

### Paso 6: Integrate with MLflow and CI/CD
...
**Esperado:** MLflow logs DVC data versions with runs, CI/CD automatically pulls data and runs pipeline, metrics validated before deployment, reproducibility enforced by CI.

**En caso de fallo:** Check secrets configured in GitHub repository settings, verify DVC remote accessible from CI runners, ensure Git credentials configured for push, check Python dependencies installed, verify metrics validation logic, inspect CI logs for DVC/MLflow errors.
