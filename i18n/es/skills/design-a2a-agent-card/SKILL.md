...
- **Opcional**: Authentication method (`none`, `oauth2`, `oidc`, `api-key`)
- **Opcional**: Supported content types beyond `text/plain` (e.g., `image/png`, `application/json`)
- **Opcional**: Capability flags (streaming, push notifications, state transition history)
