...
- **Requerido**: Codebase or module to optimize (file paths)
- **Requerido**: Value metric (performance, API clarity, bundle size, readability)
- **Opcional**: Profiling data or benchmarks showing current performance
- **Opcional**: Budget or target (e.g., "reduce bundle by 40%", "sub-100ms response")
- **Opcional**: Constraints (can't change public API, must maintain backward compat)

## Procedimiento

### Paso 1: Assay — Classify the Material
...
**Esperado:** Every significant element classified with evidence. Gold elements are identified for protection during optimization. Lead elements are prioritized by impact.

**En caso de fallo:** If profiling tools aren't available, use static analysis: function complexity (cyclomatic), dependency count, and code size as proxies. If the codebase is too large, focus on the critical path first.

### Paso 2: Refine — Amplify the Gold
...
**Esperado:** Gold and Silver elements are better tested, documented, and protected. No behavioral changes, only quality improvements.

**En caso de fallo:** If a "Gold" element reveals hidden problems during closer inspection, reclassify it. Better to be honest about value than to protect flawed code.

### Paso 3: Transmute — Convert Lead to Gold
...
**Esperado:** Measurable improvement on the target value metric. Each transmuted element performs better than its Lead predecessor while maintaining identical behavior.

**En caso de fallo:** If a Lead element resists optimization within its current interface, consider whether the interface itself is the problem. Sometimes the transmutation requires changing how the element is called, not just how it's implemented.

### Paso 4: Purge — Remove the Dross
...
**Esperado:** The codebase is lighter. Bundle size, dependency count, or code volume measurably reduced. All tests still pass.

**En caso de fallo:** If removing an element breaks something, it wasn't dross — reclassify it. If dynamic references make it hard to verify usage, add temporary logging before deletion to confirm no runtime access.

### Paso 5: Verify — Weigh the Gold
...
**Esperado:** Measurable, documented improvement on the target value metric. The codebase is demonstrably more valuable than before.

**En caso de fallo:** If overall improvement is marginal, the original code may have been better than assumed. Document what was learned — knowing that code is already near-optimal is itself valuable.
