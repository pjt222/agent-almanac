...
### Paso 1: Define Process Model and Parameters
...
**Esperado:** A fully specified stochastic model with validated parameters and a reproducible random state.

**En caso de fallo:** If parameters are inconsistent (e.g., non-stochastic matrix), correct them before proceeding. If SDE coefficients are pathological, consider a different discretization scheme.

### Paso 2: Select Simulation Method
...
**Esperado:** A selected simulation algorithm matched to the process type with appropriate tuning parameters.

**En caso de fallo:** If the chosen method is unstable (e.g., Euler-Maruyama diverging), switch to an implicit method or reduce `dt`.

### Paso 3: Implement and Run Simulation
...
**Esperado:** `n_paths` complete trajectories stored in memory, with optional function evaluations. MCMC acceptance rate is within the target range.

**En caso de fallo:** If simulation produces NaN or Inf values, reduce `dt` for SDE methods or check parameter validity. If MCMC acceptance rate is near 0% or 100%, adjust proposal scale.

### Paso 4: Apply Convergence Diagnostics
...
**Esperado:** All convergence diagnostics pass their thresholds. Trace plots show stable, well-mixing chains.

**En caso de fallo:** If R-hat > 1.1, run longer chains or improve the proposal. If ESS is very low, increase thinning or switch to a better sampler (e.g., HMC). If Geweke fails, extend burn-in.

### Paso 5: Compute Summary Statistics with Confidence Intervals
...
**Esperado:** Point estimates with associated standard errors and confidence intervals. Variance reduction (if applied) yields a VRF > 1.

**En caso de fallo:** If confidence intervals are too wide, increase `n_paths` or `n_steps`. If variance reduction worsens estimates (VRF < 1), disable it -- the control variate or antithetic scheme may not suit the problem.

### Paso 6: Visualize Trajectories and Distributions
...
**Esperado:** Publication-quality figures showing trajectory behavior, distributional convergence, and diagnostic summaries. Analytical solutions (where available) match empirical results.

**En caso de fallo:** If visualizations reveal non-stationarity or multimodality not expected from the model, revisit Steps 1-2 for parameter or method errors. If plots are cluttered, reduce the number of displayed paths or increase figure size.
