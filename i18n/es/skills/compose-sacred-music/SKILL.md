...
- **Requerido**: Purpose (compose new piece OR analyze existing piece)
...
- **Opcional**: Feast day or liturgical season (influences modal choice)
- **Opcional**: Intended performer experience level (simple syllabic vs. virtuosic melismatic)
- **Opcional**: Mode preference (if composing)

## Procedimiento

### Paso 1: Modal Selection (if composing) or Identification (if analyzing)
...
**Esperado:** Mode identified (if analyzing) or selected (if composing) with final note and characteristic range established.

**En caso de fallo:** If uncertain, default to Mode 1 (Dorian authentic, final on D). This is Hildegard's most frequent choice and offers wide melodic range.

### Paso 2: Melodic Contour and Range
...
**Esperado:** Melodic contour sketched with identified climax point, wide range planned (minimum 9th, preferably 10th-12th), and syllabic/melismatic distribution determined.

**En caso de fallo:** If melodic range feels too wide for performers, reduce climax by one step (e.g., from high D to C). Maintain the arch shape but compress the range.

### Paso 3: Text-Setting — Syllabic and Melismatic
...
**Esperado:** Text fully mapped to melody with syllabic/neumatic/melismatic choices marked. Key theological terms receive melismatic treatment. Text remains intelligible despite ornamentation.

**En caso de fallo:** If text becomes unintelligible (too much melisma), simplify non-essential words to syllabic. Retain melisma ONLY on 1-2 most important words per phrase.

### Paso 4: Neumatic Notation (Optional — for authenticity)
...
**Esperado:** (Optional) Neumatic notation sketch provided if user requests historical authenticity. Modern staff notation acceptable for performance preparation.

**En caso de fallo:** If neumatic notation is too complex, provide modern staff notation with clear phrasing marks. Hildegard's music can be performed from modern notation without loss of essential character.

### Paso 5: Liturgical Context and Performance Notes
...
**Esperado:** Liturgical use identified (when/where the piece is sung), performance notes provided (tempo, dynamics, pronunciation), and historical context clarified.

**En caso de fallo:** If liturgical context is unclear, focus on performance notes only. Hildegard's music can be performed in concert settings without strict liturgical adherence.
