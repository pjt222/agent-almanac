...
- **Requerido**: Device type (electromagnet, motor, generator, or transformer)
- **Requerido**: Performance requirements (field strength, force, torque, voltage ratio, power, efficiency target)
- **Requerido**: Operating conditions (supply voltage and current, frequency, duty cycle, ambient temperature)
- **Opcional**: Preferred core material (silicon steel, ferrite, powdered iron, air core) with B-H data
- **Opcional**: Size and weight constraints
- **Opcional**: Cost or manufacturing constraints

## Procedimiento

### Paso 1: Specify Device Requirements and Operating Conditions
...
**Esperado:** A justified topology selection with clear reasoning tied to the requirements from Step 1, including acknowledged limitations.

**En caso de fallo:** If no standard topology meets all requirements, consider a hybrid design (e.g., Halbach array for higher field with less material) or relax a secondary constraint. Document the tradeoff.

### Paso 3: Calculate Design Parameters
...
**Esperado:** Numerical values for all physical dimensions and electrical parameters, derived from electromagnetic equations with units checked at each step.

**En caso de fallo:** If the required turns do not fit in the available winding space, either increase the core size (larger window area), use finer wire (higher current density, but more heating), or reduce the performance target. If the core operates above B_max, increase the core cross-section or add turns (to reduce the flux for the same performance via a larger NI product with a larger gap).

### Paso 4: Analyze Losses and Efficiency
...
**Esperado:** All requirements met with documented margins, thermal feasibility confirmed, and the most sensitive design parameter identified.

**En caso de fallo:** If a requirement is not met, iterate by adjusting the topology (Step 2), design parameters (Step 3), or loss mitigation strategy (Step 4). If the design is thermally infeasible, consider: reducing the duty cycle, increasing the size (more surface area for cooling), switching to a higher temperature insulation class, or adding active cooling. Document each iteration.
...
- `analyze-tensegrity-system` -- análisis estructural de redes de tensión-compresión; comparte métodos de equilibrio de pretensado con el equilibrio de fuerzas electromagnéticas
