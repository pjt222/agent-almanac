...
**Esperado:** A structured profile document containing nationality status, employment classification, move type, and all fixed dates.

**En caso de fallo:** If nationality or employment status is ambiguous (e.g., dual nationality with one non-EU, or contractor vs. employee distinction unclear), escalate to a legal advisor or the destination country's embassy before proceeding. Do not guess visa requirements.

### Paso 2: Map Dependency Chain
...
**Esperado:** A dependency graph (textual or visual) showing all steps, their prerequisites, and which can run in parallel.

**En caso de fallo:** If dependencies are unclear for a specific country, search for official government sources (e.g., Germany: bmi.bund.de, Austria: oesterreich.gv.at, Switzerland: ch.ch). Do not assume dependencies transfer between countries.

### Paso 3: Create Timeline with Deadlines
...
**Esperado:** A week-by-week timeline spanning from 8-12 weeks before the move to 4-8 weeks after, with each bureaucratic step placed in its execution window.

**En caso de fallo:** If appointment availability is unpredictable (common in large German cities), build in a 2-week buffer and identify alternative offices or early-morning walk-in options.

### Paso 4: Identify Country-Specific Procedures
...
**Esperado:** A country-specific procedure list with exact office names, required forms, and typical processing times.

**En caso de fallo:** If destination is a smaller municipality, procedures may differ from the national standard. Check the specific Gemeinde/Kommune website or call their Buergerservice directly.

### Paso 5: Flag High-Risk Items
...
**Esperado:** A risk register with each high-risk item, its deadline, penalty, and mitigation strategy.

**En caso de fallo:** If penalty amounts or deadlines cannot be confirmed through official sources, mark them as "unconfirmed" and recommend direct inquiry with the relevant authority. Do not invent penalty amounts.

### Paso 6: Generate Relocation Plan Document
...
**Esperado:** A complete, structured relocation plan document ready for execution, with all items traceable back to the dependency graph and risk register.

**En caso de fallo:** If the plan is too complex for a single document (e.g., multi-country move with dependents requiring separate visa tracks), split into a master timeline and per-person sub-plans.
