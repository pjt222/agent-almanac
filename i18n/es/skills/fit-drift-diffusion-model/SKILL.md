...
- **Requerido**: Reaction time data with accuracy labels (correct/error) per trial
- **Requerido**: Subject and condition identifiers for each trial
- **Requerido**: Choice of DDM variant (basic 3-parameter, full 7-parameter, or hierarchical)
- **Opcional**: Prior distributions for Bayesian estimation (default: weakly informative)
- **Opcional**: Number of simulated datasets for parameter recovery (default: 100)
- **Opcional**: RT filtering bounds in seconds (default: 0.1 to 5.0)

## Procedimiento

### Paso 1: Prepare Reaction Time Data
...
**Esperado:** Cleaned dataframe with no RT outliers, at least 40 trials per subject-condition cell, and accuracy rates between 0.50 and 0.99.

**En caso de fallo:** If trial counts are too low, consider collapsing conditions or removing subjects with excessive missing data. If accuracy is at ceiling (>0.99) or floor (<0.55), the DDM may not be identifiable -- check task difficulty.

### Paso 2: Select DDM Variant
...
**Esperado:** A model variant selected with justification based on trial counts, subject count, and research question.

**En caso de fallo:** If unsure between variants, start with the basic model and add complexity only if residual diagnostics indicate systematic misfit (e.g., error RT distribution mismatch).

### Paso 3: Estimate Parameters
...
**Esperado:** Parameter estimates with standard errors or credible intervals. For Bayesian fits, Gelman-Rubin R-hat < 1.1 for all parameters. Drift rate typically 0.5-4.0, boundary 0.5-2.5, non-decision time 0.15-0.50s.

**En caso de fallo:** If estimation fails to converge, try: (a) tighter parameter bounds, (b) better starting values via grid search, (c) longer chains with more burn-in. If MLE hits boundary values, the model may be misspecified.

### Paso 4: Evaluate Model Fit
...
**Esperado:** QP plot shows predicted quantiles closely tracking observed quantiles for both correct and error RTs. Chi-square test is non-significant (p > 0.05), indicating adequate fit.

**En caso de fallo:** If the model systematically misses fast or slow quantiles, consider adding cross-trial variability parameters (sv, st). If error RT shape is wrong, add starting point variability (sz). Refit with the extended model.

### Paso 5: Compare Models
...
**Esperado:** A clear winner among models with BIC difference > 6, or a justified decision to retain the simpler model when the difference is < 2.

**En caso de fallo:** If models are indistinguishable (BIC difference < 2), prefer the simpler model (parsimony). If the full model wins by a large margin, ensure the basic model was not misspecified due to data issues.

### Paso 6: Validate with Parameter Recovery Simulation
...
**Esperado:** Recovery correlations r > 0.85 for all parameters, bias close to zero (< 5% of parameter range), and RMSE within acceptable bounds for the application.

**En caso de fallo:** Low recovery for a specific parameter usually means: (a) insufficient trials -- increase n_simulated_trials, (b) parameter tradeoffs -- drift rate and boundary can trade off; fix one to test recoverability, (c) flat likelihood surface -- consider reparameterization or Bayesian estimation with informative priors.
