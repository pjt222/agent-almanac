...
- **Requerido**: Region or area for the hike
- **Requerido**: Group profile (number of people, fitness level, experience)
- **Requerido**: Available time (day hike duration or number of days)
- **Opcional**: Difficulty preference (SAC T1-T6, or descriptive: easy/moderate/hard)
- **Opcional**: Elevation gain/loss tolerance (meters)
- **Opcional**: Specific peaks, huts, or destinations to include
- **Opcional**: Season and expected weather window
...
**Esperado:** A clear requirements profile with group level, time budget, elevation tolerance, and any must-include or must-avoid constraints.

**En caso de fallo:** If the group has mixed fitness levels, plan for the weakest member but identify optional extensions for stronger hikers (e.g., a peak side trip while others rest at a hut).

### Paso 2: Select Trail Candidates
...
**Esperado:** A shortlist of trail candidates with complete data sheets, all within the group's capability range.

**En caso de fallo:** If no trails match all constraints, relax the least important constraint first (typically distance before difficulty). If trail data is incomplete, note the gaps and plan to verify on-site or contact local tourism offices.

### Paso 3: Calculate Times Using Munter Formula
...
**Esperado:** Time estimates for each trail candidate, including break time. Estimates should be conservative (better to arrive early than to hike in darkness).

**En caso de fallo:** If calculated times exceed the available daylight, the route is too long. Either shorten it (find a closer end point or skip a section via transport) or split into two days. If the group is untested, use the beginner pace for the first day and adjust based on actual performance.

### Paso 4: Assess Safety
...
**Esperado:** A completed safety assessment with all hazards rated and mitigations documented. An overall GREEN/YELLOW/RED rating for the go/no-go decision.

**En caso de fallo:** If the assessment yields RED for the primary route, switch to the backup option from Step 2. If all options are RED (e.g., severe weather forecast), postpone the hike. Never override a RED safety rating for schedule convenience.

### Paso 5: Plan Logistics
...
**Esperado:** All logistics confirmed or flagged as pending. Hut reservations made. Transport to/from trailhead arranged. Emergency plan documented.

**En caso de fallo:** If huts are full, check for nearby alternatives (bivouacs, camping, lower huts with longer approach). If trailhead access is complicated (e.g., closed road), arrange alternative transport or adjust the starting point.

### Paso 6: Generate Hiking Plan
...
**Esperado:** A complete hiking plan that can be shared with all participants and left with an emergency contact. The plan should be actionable without additional research.

**En caso de fallo:** If the plan has gaps that cannot be filled before departure, document them clearly and assign someone to resolve each item. Critical safety gaps (no escape route identified, no weather check plan) must be resolved before departure.
