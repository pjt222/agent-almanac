...
- **Requerido**: Kubernetes cluster (for Litmus or Chaos Mesh)
- **Requerido**: Steady-state definition (what "normal" looks like)
- **Requerido**: Hypothesis to test (e.g., "API stays available if one pod crashes")
- **Opcional**: Observability stack (Prometheus, Grafana) to measure impact
- **Opcional**: Rollback plan

## Procedimiento

### Paso 1: Define Steady State and Hypothesis
...
**Esperado:** Clear, measurable definition of normal behavior and success criteria.

**En caso de fallo:** If you can't define steady state, observability is insufficient. Add metrics first.

### Paso 2: Set Blast Radius Limits
...
**Esperado:** Experiment has clear boundaries, won't take down entire system.

**En caso de fallo:** If blast radius is too large, narrow scope. Start with one non-critical service.

### Paso 3: Install Chaos Mesh
...
**Esperado:** Chaos Mesh or Litmus running, dashboard accessible.

**En caso de fallo:** Check RBAC permissions. Chaos tools need cluster-wide access.

### Paso 4: Create and Execute Experiment
...
**Esperado:** Pod is killed, Kubernetes restarts it, service continues with minor blip.

**En caso de fallo:** If error rate spikes or service degrades significantly, abort experiment and investigate.

### Paso 5: Analyze Results and Iterate
...
**Esperado:** Learnings captured, fixes implemented, follow-up scheduled.

**En caso de fallo:** If no action is taken post-experiment, chaos engineering becomes theater. Prioritize fixes.

### Paso 6: Graduate to Production (Carefully)
...
**Esperado:** Production experiments run during low-risk windows, with kill switch ready.

**En caso de fallo:** If production experiment causes incident, disable immediately and post-mortem.
