...
- **Requerido**: Workflow data from `put()`, `put_auto()`, or `put_merge()`
- **Opcional**: Theme name (default: `"light"`; options: light, dark, auto, minimal, github, viridis, magma, plasma, cividis)
- **Opcional**: Output target: console, file path, clipboard, or raw string
- **Opcional**: Interactive features: `show_source_info`, `enable_clicks`

## Procedimiento

### Paso 1: Extract Workflow Data
...
**Esperado:** A data frame with at least one row, containing `id`, `label`, and optionally `input`, `output`, `source_file`, `node_type` columns.

**En caso de fallo:** If the data frame is empty, no annotations or patterns were found. Run `analyze-codebase-workflow` first, or check that annotations are syntactically valid with `put("./src/", validate = TRUE)`.

### Paso 2: Select Theme and Options
...
**Esperado:** Theme names printed. Select one based on context.

**En caso de fallo:** If a theme name is not recognized, `put_diagram()` falls back to `"light"`. Check spelling.

### Paso 3: Custom Palette with `put_theme()` (Optional)
...
**Esperado:** Mermaid output with your custom classDef lines. Node shapes from `node_type` are preserved; only colors change. All node types use `stroke-width:2px` — override not currently supported via `put_theme()`.

**En caso de fallo:** If the palette object is not a `putior_theme` class, `put_diagram()` raises a descriptive error. Ensure you pass the return value of `put_theme()`, not a raw list.
...
### Paso 4: Generate Mermaid Output
...
**Esperado:** Valid Mermaid code starting with `flowchart TD` (or `LR` depending on direction). Nodes are connected by arrows showing data flow.

**En caso de fallo:** If the output is `flowchart TD` with no nodes, the workflow data frame is empty. If connections are missing, check that output filenames match input filenames across nodes.

### Paso 5: Embed in Target Document
...
**Esperado:** Diagram renders correctly in the target format. GitHub renders mermaid code fences natively.

**En caso de fallo:** If GitHub doesn't render the diagram, ensure the code fence uses exactly ` ```mermaid ` (no extra attributes). For Quarto, ensure the `knit_child()` approach is used since direct variable interpolation in `{mermaid}` chunks is not supported.
