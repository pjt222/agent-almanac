...
- **Requerido**: Skill ID (e.g., `create-skill-glyph`) and domain (e.g., `design`)
- **Requerido**: Visual concept — what the glyph should depict
- **Opcional**: Reference glyph to study for complexity level
- **Opcional**: Custom `--glow-sigma` value (default: 4)

## Procedimiento

### Paso 1: Concept — Design the Visual Metaphor
...
**Esperado:** A clear mental sketch of the shape with 2-6 planned layers.

**En caso de fallo:** If the concept is too abstract, fall back to a related concrete object. Review existing glyphs in the same domain for inspiration.

### Paso 2: Compose — Write the Glyph Function
...
**Esperado:** A working R function that returns a list of 2-6 ggplot2 layers.

**En caso de fallo:** If `ggforce::geom_circle` causes errors, ensure ggforce is installed. If coordinates are off, remember the canvas is 100x100 with (0,0) at bottom-left. Test the function interactively:
```r
...
### Paso 3: Register — Map Skill to Glyph
...
**Esperado:** The `SKILL_GLYPHS` list contains the new mapping.

**En caso de fallo:** If the build later reports "No glyph mapped for skill", double-check that the skill ID exactly matches the one in the manifest and registry.

### Paso 4: Manifest — Add Icon Entry
...
**Esperado:** Valid JSON with the new entry placed among its domain siblings.

**En caso de fallo:** Validate JSON syntax. Common mistakes: trailing comma after last array element, missing quotes.

### Paso 5: Render — Generate the Icon
...
**Esperado:** The log shows `OK: <domain>/<skill-id> (seed=XXXXX, XX.XKB)` and the WebP file exists.
...
### Paso 6: Verify — Visual Inspection
...
**Esperado:** A clear, recognizable pictogram with even neon glow on transparent background.
...
### Paso 7: Iterate — Refine if Needed
...
**Esperado:** The final icon passes all verification checks from Step 6.

**En caso de fallo:** If after 3+ iterations the glyph still doesn't read well, consider using a completely different visual metaphor (return to Step 1).
