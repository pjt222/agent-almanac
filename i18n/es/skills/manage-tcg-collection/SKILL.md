...
- **Requerido**: Card game(s) in the collection (Pokemon, MTG, FaB, Kayou, etc.)
- **Requerido**: Collection scope (entire collection, specific sets, or specific cards)
- **Opcional**: Current inventory system (spreadsheet, app, physical binder organization)
- **Opcional**: Collection goal (complete sets, competitive play, investment, nostalgia)
- **Opcional**: Budget for storage and grading supplies

## Procedimiento

### Paso 1: Establish the Inventory System
...
**Esperado:** A functional inventory system with defined fields, ready for data entry. The system matches the collection's scale — not over-engineered for a small collection, not under-powered for a large one.

**En caso de fallo:** If the ideal app isn't available for your game/platform, use a spreadsheet. The format matters less than consistency. A simple spreadsheet updated regularly beats a sophisticated app abandoned after a week.

### Paso 2: Catalog the Collection
...
**Esperado:** Every card in the collection entered with accurate condition and location data. Completion percentages known for each set being collected.

**En caso de fallo:** If the collection is too large for manual entry, prioritize: enter all rare/valuable cards first, then bulk-enter commons by set with estimated quantities. An 80% accurate inventory is far better than no inventory.

### Paso 3: Organize Physical Storage
...
**Esperado:** Every card stored appropriately for its value with location data in the inventory. Premium cards are protected, bulk cards are organized and accessible.

**En caso de fallo:** If premium storage supplies aren't available immediately, penny sleeves + top-loaders are always the minimum for any card worth >$10. Upgrade storage as supplies become available; the priority is getting valuable cards into some form of protection.

### Paso 4: Value the Collection
...
**Esperado:** A current valuation of the collection with per-card values for significant cards and aggregate values for bulk. Grading candidates identified.

**En caso de fallo:** If pricing data is stale or unavailable, note the pricing date and source. For very rare cards, check multiple sources and use the median. Never rely on a single outlier sale.

### Paso 5: Maintain and Optimize
...
**Esperado:** A living collection management system that stays current and supports informed decisions about buying, selling, grading, and trading.

**En caso de fallo:** If maintenance lapses, prioritize: update Premium tier values first, then catch up on new acquisitions. The most important thing is knowing what your most valuable cards are worth today.
