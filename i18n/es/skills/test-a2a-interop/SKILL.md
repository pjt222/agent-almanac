...
- **Requerido**: Base URL of the A2A agent under test
- **Requerido**: Authentication credentials (if the agent requires them)
- **Opcional**: Second agent URL for bidirectional interop testing
- **Opcional**: Specific skills to test (default: all skills in the Agent Card)
- **Opcional**: Test timeout per task (default: 60 seconds)
- **Opcional**: Output format for the conformance report (`json`, `markdown`, `junit`)

## Procedimiento

### Paso 1: Fetch and Validate Agent Cards
...
**Esperado:** Agent Card passes all structural validation checks.

**En caso de fallo:** Record each validation failure with the specific field and reason. Do not abort; continue testing other aspects. An invalid Agent Card is itself a test result.

### Paso 2: Send Test Tasks Covering All Lifecycle States
...
**Esperado:** All lifecycle state transitions work correctly. Tasks complete successfully, cancel cleanly, and multi-turn interaction functions when supported.

**En caso de fallo:** Record the specific state transition that failed, the expected state, and the actual state. Include the full JSON-RPC response in the report for debugging.

### Paso 3: Validate SSE Streaming Responses
...
**Esperado:** SSE stream delivers correctly formatted events in the right sequence, ending with a final terminal event.

**En caso de fallo:** If SSE is advertised but the endpoint returns a non-SSE response, record as a conformance failure. If events arrive out of order, record the sequence. If the stream never terminates, record a timeout.

### Paso 4: Test Error Handling and Edge Cases
...
**Esperado:** All error conditions return appropriate JSON-RPC error codes without crashing the server.

**En caso de fallo:** Record each error handling test that fails. Server crashes during error testing are critical failures that must be fixed before deployment.

### Paso 5: Generate Interoperability Conformance Report
...
**Esperado:** A complete conformance report with pass/fail results, conformance level, and actionable recommendations.

**En caso de fallo:** If the report generation itself fails, output raw test results to stdout as a fallback. The test data should never be lost due to a reporting error.
