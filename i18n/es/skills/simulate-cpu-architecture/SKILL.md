...
- **Requerido**: Processor complexity target -- 4-bit, 8-bit, or 16-bit data width; number of general-purpose registers (2-16)
- **Requerido**: Minimum instruction set -- at least: load, store, add, subtract, bitwise AND/OR, branch, halt
- **Opcional**: Addressing modes beyond direct (immediate, register-indirect, indexed)
- **Opcional**: Additional instructions (multiply, shift, compare, jump-and-link for subroutines)
- **Opcional**: Memory size and word size
- **Opcional**: Pipeline stages (single-cycle, multi-cycle, or pipelined) -- default is multi-cycle
- **Opcional**: Implementation medium -- software simulation (Python/JS), HDL (Verilog/VHDL), or paper walkthrough

## Procedimiento

### Paso 1: Define the Instruction Set Architecture
...
**Esperado:** A complete ISA specification where every instruction has a unique opcode, well-defined operand fields, an unambiguous RTL description, and documented flag effects. The instruction encoding must be decodable without ambiguity.

**En caso de fallo:** If the instruction word is too narrow to encode all needed fields, either widen the instruction, reduce the register count, use variable-length instructions (more complex decoding), or split instructions into sub-operations. If opcodes collide, reassign the encoding.

### Paso 2: Design the Datapath
...
**Esperado:** A complete datapath diagram (as a component table and mux table) where every instruction in the ISA has a viable path for its data to flow from source to destination through the ALU, register file, and memory.

**En caso de fallo:** If an instruction cannot be executed with the current datapath (e.g., no path from memory to a register for LOAD), add the missing multiplexer or data path. Walk through each instruction's RTL operation and trace the required signal flow through the datapath.

### Paso 3: Design the Control Unit
...
**Esperado:** A control unit (combinational or FSM) that generates the correct control signal values for every instruction at every phase, with no conflicting signals (e.g., MemRead and MemWrite both active simultaneously on the same memory).

**En caso de fallo:** If a control signal conflict exists, the phases are not properly separated. Ensure that load and store instructions access memory in different phases or that the memory interface supports simultaneous read and write on separate ports. If the FSM has too many states, check whether some instructions share phases and can be merged.

### Paso 4: Implement the Fetch-Decode-Execute Cycle
...
**Esperado:** A fully connected processor where the control FSM drives the datapath through the correct sequence of phases for each instruction type, and all state transitions occur synchronously on the clock edge.

**En caso de fallo:** If the processor hangs (never reaches HALT) or produces incorrect results, the most likely cause is a control signal error in one specific phase. Use Step 5's cycle-by-cycle trace to isolate the failing cycle. If the PC does not increment correctly, check the FETCH phase wiring. If the wrong register is written, check the register address field extraction from IR.

### Paso 5: Simulate a Small Program and Verify
