...
**Esperado:** Script structure with clear_scene() and main() functions
**En caso de fallo:** Review Python syntax, check bpy import works in Blender Python environment
...
**Esperado:** Objects appear in scene with correct names and positions
**En caso de fallo:** Check operator syntax, verify coordinates, ensure no naming conflicts
...
**Esperado:** Materials visible in shader editor with proper node connections
**En caso de fallo:** Check node types exist, verify link syntax, ensure color values in [0,1] range
...
**Esperado:** Three lights with appropriate intensities and positions
**En caso de fallo:** Adjust energy values for render engine (Cycles vs EEVEE), check rotation format
...
**Esperado:** Camera positioned with correct focal length and DOF settings
**En caso de fallo:** Use simpler rotation method if track_to fails, verify lens units
...
**Esperado:** World shader with HDRI or solid background configured
**En caso de fallo:** Skip HDRI loading if file missing, use Background node alone with color
...
**Esperado:** Render settings configured, ready for rendering
**En caso de fallo:** Check engine name spelling, verify resolution values are positive integers
...
**Esperado:** Objects organized in named collections for easier management
**En caso de fallo:** Check collection already exists before creating, handle orphaned objects
