...
- **Requerido**: Description of the damage (what broke, when, how severely)
- **Requerido**: Current system state (what's still working, what's not)
- **Opcional**: Root cause (if known — may not be clear yet)
- **Opcional**: Pre-damage system state (for comparison)
- **Opcional**: Available repair resources (time, people, budget)
- **Opcional**: Urgency (is the system actively degrading or stable-but-damaged?)

## Procedimiento

### Paso 1: Triage — Assess and Classify Wounds
...
**Esperado:** A complete wound inventory classified by severity, with a prioritized repair order that accounts for wound interactions.

**En caso de fallo:** If triage takes too long (the system is actively degrading), skip detailed classification and focus on: "What is the single most critical thing to stabilize?" Fix that first, then return to full triage.

### Paso 2: Emergency Stabilization
...
**Esperado:** The system is stable (not actively degrading) even if degraded. Damage is contained and not spreading. Evidence is preserved for root cause analysis.

**En caso de fallo:** If stabilization fails (damage continues spreading despite containment), escalate to full system fallback: activate disaster recovery, switch to backup system, or gracefully degrade to minimal viable operation. Stabilization that takes too long becomes the disaster.

### Paso 3: Build Repair Scaffolding
...
**Esperado:** A repair environment with diagnostic capability, a sequenced repair plan, and awareness of scar tissue risk.

**En caso de fallo:** If setting up a proper repair environment is too slow (system urgency demands immediate production changes), apply changes directly but with extreme discipline: one change at a time, tested by the available means, rolled back if it doesn't help.

### Paso 4: Execute Progressive Rebuild
...
**Esperado:** Critical and serious wounds are repaired with verified fixes. Emergency patches are removed. The system is restored to functional operation.

**En caso de fallo:** If a repair attempt fails or causes regression, roll back to the previous state and reassess. If multiple repair attempts fail for the same wound, the damage may be too deep for local repair — consider whether the affected component needs full replacement rather than repair (see `dissolve-form`).

### Paso 5: Manage Scar Tissue and Strengthen
...
**Esperado:** Scar tissue is managed (removed, replaced, or accepted with documentation). The system is not only repaired but more resilient than before the damage. Learnings are captured for future incidents.

**En caso de fallo:** If scar tissue management is deprioritized ("it works, don't touch it"), schedule it explicitly. Unmanaged scar tissue accumulates and eventually contributes to the next incident. If the root cause can't be identified, strengthen detection and recovery speed as compensating controls.
