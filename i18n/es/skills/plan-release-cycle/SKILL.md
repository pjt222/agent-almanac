...
- **Requerido**: Target version number (e.g., v2.0.0)
- **Requerido**: Desired release date or release window
- **Requerido**: List of planned features or scope (backlog, roadmap, or description)
- **Opcional**: Team size and availability
- **Opcional**: Release strategy preference (calendar-based or feature-based)
- **Opcional**: Regulatory or compliance requirements affecting release
- **Opcional**: Previous release velocity or cycle duration data

## Procedimiento

### Paso 1: Determine Release Strategy
...
**Esperado:** Release strategy documented with rationale matching project context.

**En caso de fallo:** If the team cannot agree on a strategy, default to calendar-based with a feature-priority list. Time-boxing forces prioritization decisions.

### Paso 2: Define Milestones
...
**Esperado:** Milestone table with dates, durations, and descriptions for each phase.

**En caso de fallo:** If the timeline is too compressed (stabilization < 1 week), either extend the release date or reduce scope. Never skip stabilization.

### Paso 3: Set Feature Freeze Criteria
...
**Esperado:** Feature freeze rules documented with exception process and prioritized feature list.

**En caso de fallo:** If P0 features are at risk of missing the freeze date, escalate immediately. Options: extend development phase, split the feature into a smaller deliverable, or defer to a point release (v2.0.1).

### Paso 4: Plan Release Candidate Process
...
**En caso de fallo:** If the RC process is skipped (pressure to release), document the risk. Untested releases have higher rollback probability.

### Paso 5: Define Go/No-Go Checklist
...
**Esperado:** Go/no-go checklist with clear pass/fail criteria and decision rules.

**En caso de fallo:** If the go/no-go meeting results in no-go, identify the blocking items, assign owners, set a new target date (typically 1-2 weeks later), and update the release plan.

### Paso 6: Document Rollback Plan
...
**Esperado:** Rollback plan documented with triggers, procedure, communication template, and point release policy. Complete RELEASE-PLAN.md written.

**En caso de fallo:** If rollback is not feasible (e.g., database migration already applied), document the forward-fix procedure instead. Every release should have a recovery path.
