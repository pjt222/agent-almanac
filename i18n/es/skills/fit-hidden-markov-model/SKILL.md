...
### Paso 1: Define Hidden States and Observation Model
...
**Esperado:** A clearly specified HMM architecture with `K` states, a chosen emission family, and clean observation data of length `T >> K^2`.

**En caso de fallo:** If data contains missing values, impute or remove affected segments. If `T` is too small relative to `K`, reduce `K` or acquire more data.

### Paso 2: Initialize Parameters
...
**Esperado:** `n_restarts` sets of valid initial parameters, with at least one data-driven initialization.

**En caso de fallo:** If K-means fails to converge, use purely random initialization with more restarts. If covariance matrices are singular, add the regularization constant to the diagonal.

### Paso 3: Run Baum-Welch EM for Parameter Estimation
...
**Esperado:** Monotonically non-decreasing log-likelihood across iterations, converging within `max_iterations`. Final parameters are valid (stochastic matrices, positive-definite covariances).

**En caso de fallo:** If log-likelihood decreases, there is a bug in the E-step or M-step -- verify formulas. If convergence is very slow, try better initialization or increase `max_iterations`. If covariance becomes singular, increase regularization.

### Paso 4: Apply Viterbi Decoding for Most Likely State Sequence
...
**Esperado:** A single most-likely state sequence of length `T` with each entry in `{1,...,K}`. The Viterbi log-probability should be less than or equal to the total log-likelihood.

**En caso de fallo:** If the Viterbi path has log-probability of negative infinity, some transition or emission probability is zero where it should not be. Add floor values to prevent log(0).

### Paso 5: Perform Model Selection (BIC/AIC Across Model Orders)
...
**Esperado:** A clear minimum in BIC/AIC identifying the optimal number of hidden states. The selected model should have converged and have interpretable state meanings.

**En caso de fallo:** If no clear minimum exists (monotonically decreasing BIC), the model may be misspecified -- consider a different emission family. If all models have poor log-likelihood, the data may not follow an HMM structure.

### Paso 6: Validate with Held-Out Data and Posterior Decoding
...
**Esperado:** Held-out log-likelihood is reasonably close to training log-likelihood (no severe overfitting). Viterbi and posterior decoding agree on 90%+ of time steps. States have distinct, interpretable emission distributions.

**En caso de fallo:** If held-out likelihood is much worse than training, the model is overfitting -- reduce `K` or increase regularization. If states are not interpretable, try different initializations or a different emission family.
