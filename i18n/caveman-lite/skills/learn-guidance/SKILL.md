...
**Got:** A learning path the person can see and understand. It should feel manageable — not overwhelming. The person should be able to point to any milestone and understand why it is there.

**If fail:** If the path feels too long, the goal may be too ambitious for the available time — discuss scope reduction. If the path feels too short, the topic may be simpler than expected — or the milestones are too coarse and need decomposition.
...
**Got:** The person progresses through the material with comprehension, not exposure. They should be able to explain each concept in their own words before moving to the next. The pace feels right — not rushed, not dragged.

**If fail:** If they are struggling, slow down and check for missing prerequisites. If they are breezing through, speed up — do not waste their time on what they already grasp. If the material itself is confusing (bad documentation), provide a clearer explanation and note the resource quality for future reference.

### Step 4: Test — Check Understanding

Verify learning with questions that require application, not recall.
...
4. Ask debugging questions: "This code has a bug related to what we learned — can you spot it?"
...
**Got:** The testing reveals whether the person has a working mental model or surface-level recall. Working models can handle variations; surface recall cannot. The testing should feel like a collaborative exercise, not an exam.

**If fail:** If the person cannot answer application questions, the learning was too passive — they need hands-on practice before more material. If they answer recall questions but not application questions, the concepts were understood individually but not integrated — focus on connections between concepts.
...
**Got:** The person leaves with clear understanding of what they learned, what they can practice, and what comes next. The session has a clean closing, not an abrupt stop.

**If fail:** If the person cannot state a key takeaway, the session covered too much or too little stuck. Identify the one concept that most needs reinforcement and focus the review on that. If they have no motivation for independent practice, the learning path may need to be more self-contained (all learning within sessions).
...
- [ ] Testing used application questions, not recall
...
- **All theory, no practice**: Understanding requires doing, not hearing. Every milestone should include a practice element
