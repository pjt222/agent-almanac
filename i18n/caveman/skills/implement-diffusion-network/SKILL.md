...
2. Pre-compute derived quantities used during training + sampling:
...
3. Implement forward noising function (q-sample):
...
2. Define residual block with time conditioning:
...
3. Assemble U-Net with encoder, bottleneck, decoder:
...
4. Verify architecture accepts inputs of target resolution:
...
2. Configure optimizer + learning rate schedule:
...
3. Run training loop with logging:
...
2. Assess sample diversity (check mode collapse):
