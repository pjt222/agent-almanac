...
Monitor service availability from external vantage points. Prevent SSL cert expirations.

## When Use

- Monitoring customer-facing endpoints (APIs, websites)
- Tracking SSL cert expiration
...
- **Optional**: Multiple geo probe locations
...
**Got:** Blackbox Exporter running + accessible on port 9115.

**If fail:** Check firewall rules, ensure config volume mounted correct.

### Step 2: Configure Blackbox Modules

Make `blackbox.yml` with various probe types.
...
**If fail:** Check Prometheus logs for scrape errors. Verify Blackbox Exporter reachable.
...
**Got:** Alerts fire when endpoints unreachable or SSL certs expiring.
...
**If fail:** Check Prometheus data source in Grafana, verify metrics being scraped.
...
**Got:** Public status page shows current service status + incidents.

**If fail:** Ensure status page URL reachable by customers, not behind VPN.
...
- [ ] SSL cert expiry alerts configured (14-day warning)
- [ ] Alerts tested (simulate endpoint down, check alert fires)
- [ ] Grafana dashboard shows uptime + SSL expiry
- [ ] Status page accessible to customers
- [ ] Alert notifications reach on-call engineers

## Pitfalls

- **Internal-only checks**: Blackbox Exporter inside cluster cannot detect external DNS/routing issues. Deploy probes in multiple clouds/regions.
- **Too frequent scraping**: Checking every 10 seconds generates load. 30-60s usually sufficient.
- **No SSL monitoring**: Expired certificates are embarrassing + preventable. Always monitor.
- **Status page not automated**: Manually updating status pages during incidents wastes time. Automate from Prometheus alerts.
- **False positives**: Single failed check should not alert. Use `for: 2m` to avoid transient network blips.
