...
description: >
  Sharpen knives w/ whetstones, field stones, improvised abrasives.
  Blade anatomy, bevel, whetstone tech (coarse→fine), strop, sharpness
  test, field methods, edge maintenance. Use → fingernail test fails,
  excess pressure, before trip, after heavy use, visible nicks/rolled edge.
  Bushcraft, folding, garden tools.
license: MIT
allowed-tools: Read
metadata:
...
- `make-fire` — sharp knife → tinder, kindling, feathersticks; companion skill
- `forage-plants` — sharp blade → clean sustainable harvest cuts
- `maintain-hand-tools` — garden blades (hori-hori, secateurs) similar; this = deeper knife technique
