...
- **Tools** (`tools/`): 10 operator utilities (4 Node.js, 3 shell, 3 Python) catalogued in `tools/_registry.yml`, none shipped in the published package and none a merge gate. Each carries a self-test: 9 run in the non-required `tools-verify` job, 1 skipped there with the reason recorded in the registry. 3 act beyond this checkout: `wirecap.py` stands up a local HTTP endpoint to capture a session's request body, `merge-dependabot.sh` merges pull requests through `gh`, and `agent-report.mjs` reads session transcripts under the Claude Code projects directory, outside this repository.
