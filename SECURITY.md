...
- **Tools** (`tools/`): 11 operator utilities (4 shell, 4 Node.js, 3 Python) catalogued in `tools/_registry.yml`, none shipped in the published package and none a merge gate. Each carries a self-test: 10 run in the non-required `tools-verify` job, 1 skipped there with the reason recorded in the registry. 4 act beyond this checkout: `wirecap.py` stands up a local HTTP endpoint to capture a session's request body, `merge-dependabot.sh` merges pull requests through `gh`, `agent-report.mjs` reads session transcripts under the Claude Code projects directory, outside this repository, and `watch-checks.sh` reads check results from the GitHub API through `gh`.
