...
- **Skills, Agents, Teams, Guides**: mostly Markdown and YAML, plus **18 files that are not** (.bib, .json, .py, .webp) — 2 of them executable scripts: `skills/consult-a-decision-oracle/references/separation.py`, `skills/verify-web-app-runtime/scripts/verify_runtime.py`. All of it ships. 231 of 373 skills (~62%) declare `Bash` in their `allowed-tools`, meaning they instruct AI agents to execute shell commands when followed. Review any skill before letting an agent execute it.
